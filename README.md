...
> 🧠 **Your files. Your control.** — 面向家庭和小团队的自托管私有云存储

MnemoNAS 是一个简单可靠、好看好用的开源 NAS 系统。数据在自己手里，优先服务家庭和小团队的日常文件管理。
...
- 🚀 **快速部署**：提供 Docker Compose 和 Ubuntu systemd 部署路径，适合闲置笔记本或小型服务器
...
### Ubuntu 笔记本 / systemd（推荐用于长期运行）

从 [Releases](https://github.com/seanbao/mnemonas/releases) 下载 Linux release 包，在闲置 Ubuntu 笔记本或小主机上安装为 systemd 服务：
...
默认会安装到 `/usr/local/bin`，配置写入 `/etc/mnemonas/config.toml`，数据放在 `/srv/mnemonas`，Web UI 监听 `http://<ubuntu-laptop-ip>:8080`。首次登录密码在 `/srv/mnemonas/.mnemonas/initial-password.txt`。

详见 [Ubuntu 笔记本部署指南](docs/ubuntu-laptop-deployment.md)。
...
| [Ubuntu 笔记本部署](docs/ubuntu-laptop-deployment.md) | 闲置 Ubuntu 笔记本或小主机的 systemd 长期运行指南 |
