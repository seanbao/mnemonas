...
[`client/`](client/README.md) 包含 Android、Linux 和 Windows 的 Flutter 客户端工程，Android 是首个可用平台目标。当前源码已接入服务器连接、带 revision/CAS 隔离的 Bearer 会话与 refresh token 轮换、文件浏览、有界文件名搜索、可恢复上传下载、重命名、移动、复制、两阶段安全删除、回收站恢复与精确永久删除、账户管理和问题反馈。
