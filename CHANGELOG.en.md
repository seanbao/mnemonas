...
- Added a Flutter client project for Android, Linux, and Windows, with Android as the first usable-platform target. The current source implements server validation, Bearer sessions with single-use refresh-token rotation, secure session storage, file browsing and core file operations, version history and administrator restore, bounded filename search, Trash, recoverable upload and download with a durable foreground ledger, Android Storage Access Framework import and export, system-back hierarchy, a fail-closed Release signing gate, account flows, and an Issue feedback entry. Limited manual validation has completed on an API 35 emulator and an API 36 physical device. Android background execution and notifications, production-key custody, a distributable candidate, the complete device and failure matrix, and native Linux and Windows build and runtime validation remain incomplete; no usable client version has been published.
