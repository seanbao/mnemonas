...
run_webdav_docs_avoid_placeholder_password_test() {
    ...
    assert_file_contains "$REPO_ROOT/docs/reverse-proxy-setup.md" "未来公开发布的 WebDAV 挂载应优先使用 \`auth_type=users\`"
    assert_file_contains "$REPO_ROOT/docs/reverse-proxy-setup.en.md" "Future public-release WebDAV mounts should prefer \`auth_type=users\`"
