...
touch README.md CODE_OF_CONDUCT.md CODE_OF_CONDUCT.zh-CN.md SUPPORT.md SUPPORT.en.md .github/ISSUE_TEMPLATE/bug_report.yml .github/dependabot.yml Makefile Dockerfile go.mod dataplane/src/lib.rs tools/proto-gen/Cargo.toml
...
printf '%s\n' '# feedback policy changed' >SUPPORT.md
printf '%s\n' '# feedback policy changed' >SUPPORT.en.md
...
printf '%s\n' '#!/usr/bin/env bash' 'exit 0' >scripts/verify-published-release.sh
printf '%s\n' '#!/usr/bin/env bash' 'exit 0' >scripts/test-published-release.sh
printf '%s\n' '#!/usr/bin/env bash' 'exit 0' >scripts/public-go-live-smoke.sh
...
assert_file_contains "$output_dir/plan.out" "[hardening-commit-plan] grouped 24 changed file(s)"
...
assert_file_contains "$output_dir/plan.out" "SUPPORT.md"
assert_file_contains "$output_dir/plan.out" "SUPPORT.en.md"
...
assert_file_contains "$output_dir/plan.out" "scripts/verify-published-release.sh"
assert_file_contains "$output_dir/plan.out" "scripts/test-published-release.sh"
assert_file_contains "$output_dir/plan.out" "scripts/public-go-live-smoke.sh"
...
assert_file_contains "$output_dir/checks.out" "GOTOOLCHAIN=local CGO_ENABLED=1 bash ./scripts/with-test-dataplane.sh go test -timeout=30m -p=3 -race ./internal/api ./internal/share ./internal/webdav"
...
assert_file_contains "$output_dir/no-manual.out" "[hardening-commit-plan] grouped 23 changed file(s)"
