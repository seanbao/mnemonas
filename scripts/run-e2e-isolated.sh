#!/usr/bin/env bash
# Start an isolated MnemoNAS backend and run the shell acceptance tests against it.
...
E2E_ROOT="${MNEMONAS_E2E_ROOT:-/tmp/mnemonas-acceptance-e2e}"
...
BASE_URL=""
READY_ATTEMPTS="${MNEMONAS_E2E_READY_ATTEMPTS:-180}"
...
wait_for_backend_ready() {
  for _ in $(seq 1 "$READY_ATTEMPTS"); do
    ...
    if ! kill -0 "$backend_pid" 2>/dev/null; then
      ...
      local status=$?
      set -e
      echo "isolated E2E backend exited before it was ready (status $status)" >&2
      show_backend_logs
      exit "$status"
    fi
  ...
  echo "isolated E2E backend did not become ready at $BASE_URL" >&2
  show_backend_logs
  return 1
}
...
require_safe_e2e_root
require_safe_tcp_host "$NASD_HOST" "MNEMONAS_E2E_NASD_HOST"
...
require_positive_integer "$READY_ATTEMPTS" "MNEMONAS_E2E_READY_ATTEMPTS"
...
wait_for_backend_ready
echo "Running shell E2E tests against isolated backend: $BASE_URL"
...
BASE_URL="$BASE_URL" \
  ...
  "$PROJECT_ROOT/scripts/e2e-test.sh" "$@"
