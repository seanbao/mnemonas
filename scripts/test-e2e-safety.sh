...
run_backup_restore_docs_contract_test() {
	...
	assert_file_contains "$REPO_ROOT/docs/api-reference.md" 'restore-preview'
	assert_file_contains "$REPO_ROOT/docs/api-reference.md" '不创建目标数据或写入恢复历史'
	assert_file_contains "$REPO_ROOT/docs/api-reference.en.md" 'restore-preview'
	assert_file_contains "$REPO_ROOT/docs/api-reference.en.md" 'does not create target data or write restore history'
	assert_file_contains "$REPO_ROOT/docs/backup-guide.md" '维护页会要求当前目标目录和配置选项已经成功预览'
	assert_file_contains "$REPO_ROOT/docs/backup-guide.md" '没有失败预检项后才允许开始恢复'
	assert_file_contains "$REPO_ROOT/docs/backup-guide.en.md" 'requires a successful preview that still matches the current target and config option'
	assert_file_contains "$REPO_ROOT/docs/backup-guide.en.md" 'has no failed preflight checks before enabling restore'
	assert_file_contains "$REPO_ROOT/docs/api-reference.md" '服务端会在写入前重新运行相同恢复预检'
	assert_file_contains "$REPO_ROOT/docs/api-reference.en.md" 'The server reruns the same restore preflight before writing'
	assert_file_contains "$REPO_ROOT/docs/backup-guide.md" '最近一次报告会持久化为'
	assert_file_contains "$REPO_ROOT/docs/backup-guide.md" 'last_restore_verify'
	assert_file_contains "$REPO_ROOT/docs/backup-guide.en.md" 'The latest report is persisted as'
	assert_file_contains "$REPO_ROOT/docs/backup-guide.en.md" 'last_restore_verify'
	assert_file_contains "$REPO_ROOT/docs/backup-guide.md" '批量恢复 API 在写入任何目标目录前会重新执行整批预检'
	assert_file_contains "$REPO_ROOT/docs/backup-guide.md" '不会写入任何目标数据'
	assert_file_contains "$REPO_ROOT/docs/backup-guide.en.md" 'reruns preflight for the full batch before writing any target directory'
	assert_file_contains "$REPO_ROOT/docs/backup-guide.en.md" 'writes no target data'
	assert_file_contains "$REPO_ROOT/docs/api-reference.md" '在每个成功恢复后运行只读'
	assert_file_contains "$REPO_ROOT/docs/api-reference.md" 'restore-verify'
	assert_file_contains "$REPO_ROOT/docs/api-reference.en.md" 'runs read-only'
	assert_file_contains "$REPO_ROOT/docs/api-reference.en.md" 'after every successful restore'
