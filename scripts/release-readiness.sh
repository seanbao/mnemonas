...
check_release_workflow() {
	...
	require_file_contains "$path" "uses: actions/upload-artifact@v4"
	...
	require_workflow_job_contains "$path" "release" "needs: [build, docker]"
	...
	require_workflow_job_contains "$path" "release" "uses: actions/download-artifact@v4"
	require_workflow_job_contains "$path" "release" "path: dist"
	require_workflow_job_contains "$path" "release" "merge-multiple: true"
	require_workflow_job_contains "$path" "release" "sha256sum ./*.tar.gz > checksums.txt"
	require_workflow_job_contains "$path" "release" "--version \"\$GITHUB_REF_NAME\""
	require_workflow_job_contains "$path" "release" "--repository \"\$GITHUB_REPOSITORY\""
	require_workflow_job_contains "$path" "release" "--check-image"
	require_workflow_job_contains "$path" "release" "dist/*.tar.gz"
	require_workflow_job_contains "$path" "release" "dist/checksums.txt"
}
...
if [[ "$CHECK_CHECKLIST" -eq 1 ]]; then
	...
	require_file_contains "CHANGELOG.md" "./scripts/release-go-live-check.sh"
	require_file_contains "CHANGELOG.en.md" "./scripts/release-go-live-check.sh"
