...
- Release artifact verifier 在执行 checksum 前拒绝不安全的 checksum 路径、控制字符路径、空白字符路径、符号链接归档、下载目录中的未知条目、特殊归档条目、重复条目、归档成员控制字符路径、归档成员空白字符路径、反斜杠路径和歧义路径段，并会校验显式或从归档名推断出的 release version 是否符合 Docker/GHCR 镜像标签约束
