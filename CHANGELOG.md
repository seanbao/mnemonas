...
#### Flutter 客户端（开发中）
- 新增面向 Android、Linux 和 Windows 的 Flutter 客户端工程，以 Android 作为首个可用平台目标。当前源码已实现服务器验证、Bearer 会话与单次 refresh token 轮换、安全会话存储、文件浏览与核心文件操作、版本历史与管理员恢复、有界文件名搜索、回收站、带持久前台账本的可恢复上传与下载、Android Storage Access Framework 导入与导出、系统返回层级、失败关闭的 Release 签名门禁、账户流程及 Issue 反馈入口；API 35 模拟器与 API 36 真机已完成有限人工验证。Android 后台执行与通知、正式密钥托管、可分发候选包、完整设备与故障矩阵，以及 Linux 和 Windows 原生构建与运行验证仍未完成；客户端尚未发布任何可用版本。
...
- `make test`、`make quick-check`、`make coverage`、torture 测试和 hardening 分组规划命令会使用 30 分钟 Go 包级超时；完整 race 门禁将包级并发限制为 3，CI 为 Go job 保留 60 分钟总时限，避免重负载包因资源竞争和详细日志开销误超时
