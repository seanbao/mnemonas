...
- `scripts/docker-smoke.sh` 会在启动容器前拒绝空值、以 `-` 开头或包含空白/控制字符的镜像引用，避免 Docker smoke 把镜像名误解释为 Docker 运行参数；HTTP 探测现在带连接和总耗时上限，并以 `/health` HTTP 成功响应判定服务可达，避免半开连接或空响应体拖住容器烟测
