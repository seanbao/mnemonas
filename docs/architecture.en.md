...
- When the app enters the `hidden`, `paused`, or `detached` lifecycle state, the controller synchronously claims active transfer-center tasks before cancelling their network requests or Android document copies. Durable tasks retain their app-private payload and partial progress and become paused or await a newly selected destination. Transient file previews and historical-version previews or downloads are cancelled and their partial download files are cleaned; the separate SAF upload-preparation dialog also cancels native import and cleans its partial file. Returning to the foreground only resets the lifecycle-cycle guard and never resumes a task automatically.
- Version history is available to a versioned regular file with a canonical path and concrete-read permission; the directory entry does not need to contain a content hash in advance. The version API returns and establishes the current 64-character lowercase BLAKE3 identity for that path. If the directory entry already carries an identity, the two must also match. The response strictly validates the path, continuous sequence, hash, size, timestamp, and comment. The current-version card displays identity and metadata only, while preview and download actions are limited to historical versions; current-file actions remain on the Files page. A historical download remains an authenticated stream and requires a strong ETag exactly equal to the selected version hash and a matching length; no Bearer token enters an external open or save URL. Before an administrator restore, the client reloads the parent directory and history and checks both the current identity established by the initial version response and the selected history entry. The restore `POST` is never replayed automatically. A connection loss, timeout, unreadable response, or `5xx` becomes an unconfirmed result. The server contract does not expose a current-identity CAS parameter, so a TOCTOU window remains between the pre-submit check and the write request; the server preserves the then-current content as a safety version during restore.

This section describes the current source architecture only. Both durable upload and download ledgers are driven by the foreground Dart coordinator. An Android native background executor, notification actions, and cross-process task leases remain incomplete. The target background boundary uses user-initiated data transfer (UIDT) jobs on Android 14 and later, a WorkManager long-running fallback on Android 7 through 13, and one application-scoped FlutterEngine for Dart coordination. Before that boundary can be connected, the task ledger must become a transactional store with revision/CAS, durable task commands, and fencing tokens. A SAF document provider may retain an empty or partial document after native-copy failure; the client does not automatically delete an external document whose ownership cannot be confirmed. Desktop destination publication also lacks a cross-process no-replace primitive, durable publication journal, and directory synchronization. The Flutter client remains under development and no usable version has been published. Release builds now have fail-closed signing validation, and an Android 16 physical device covers basic installation, system-back behavior, session recovery after process restart, and a same-signature Debug update. Production-key custody, a controlled Release candidate, the complete Android device/failure matrix, and validated Linux and Windows distributable builds remain incomplete.
