...
Latest local full-validation snapshot: validation target `abcbbb9c8fac`; `GOTOOLCHAIN=local timeout 90m ./scripts/verify-changed.sh --base master` passed, covering diff whitespace, secret-leak scanning, workflow/YAML/script gates, the release-readiness checklist summary-scope gate, the `make release-readiness` entry-point baseline, `make check`, toolchain consistency, Go/Rust/frontend dependency security scans, example config validation, public-access templates, protobuf regeneration stability, Rust fmt/test/clippy, proto-gen fmt/test/clippy, frontend lint/typecheck/unit/build, 377 Playwright E2E cases, Docker build, Docker image `sha256:8b3af92bc7b52a86bcbfca8d4a34533b68d90dea14a4bba233dfef44aa103802`, and Docker smoke. The Docker smoke used the Docker-assigned loopback port `http://127.0.0.1:32790`.
