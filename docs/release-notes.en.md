...
Latest local full-validation snapshot: validation target `2821b6491036`; `GOTOOLCHAIN=local timeout 90m ./scripts/verify-changed.sh --base master` passed, covering diff whitespace, secret-leak scanning, workflow/YAML/script gates, the release-readiness checklist summary-scope gate, Release workflow structural gates, the hardening progress `make release-readiness` row-level validation-target gate, the `make release-readiness` entry-point baseline, `make check`, toolchain consistency, Go/Rust/frontend dependency security scans, example config validation, public-access templates, protobuf regeneration stability, Rust fmt/test/clippy, proto-gen fmt/test/clippy, frontend lint/typecheck/unit/build, 377 Playwright E2E cases, Docker build, Docker image `sha256:23d73cf54a74f7eba8e82306b58e6775bc914d9ccf530241609e5cac274e90a8`, and Docker smoke. The Docker smoke used the Docker-assigned loopback port `http://127.0.0.1:32793`.
