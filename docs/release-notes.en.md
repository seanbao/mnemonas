...
- The Maintenance completed-restore dialog can copy a restore cutover record with the target path, read-only verification result, cutover steps, pre-cutover confirmation, and rollback checklist. Restore reports now add explicit findings when the latest restore is complete but lacks matching read-only verification, when available read-only verification predates restore completion, or when it belongs to a different restore target, avoiding stale verification being read as current evidence. Batch restore results list cross-directory cutover candidates and conflict-disposition records, and include job names, backup targets, retention-policy status, candidate paths, read-only verification conclusions, verification error details, disposition guidance, and config-file retention requirements in the copyable result record for ticket or duty-log handoff.
...
- Key disposition entry points in Shares, version history, Trash, and Maintenance write activity review records for share disable, deletion, re-enable, version restore, Trash restore, and backup restore execution results. Activity review history now immediately shows newly matching records after disposition when they match the current filter, making accidental sharing, deletion, and restore follow-up traceable.
...
Latest local full-validation snapshot: validation target `3e187452118e`; `GOTOOLCHAIN=local timeout 90m ./scripts/verify-changed.sh --base master` passed, covering diff whitespace, secret-leak scanning, workflow/YAML/script gates, the restore-report increment that adds explicit findings when the latest restore is complete but lacks matching read-only verification, when available read-only verification predates restore completion, or when it belongs to a different restore target, the Activity review cache update that immediately inserts updated records matching the current history filter, the reverse-proxy WebDAV verification documentation contract gate, the release-readiness checklist summary-scope gate, Release workflow structural gates, the hardening progress `make release-readiness` row-level validation-target gate, the `make release-readiness` entry-point baseline, `make check`, toolchain consistency, Go/Rust/frontend dependency security scans, example config validation, public-access templates, protobuf regeneration stability, Rust fmt/test/clippy, proto-gen fmt/test/clippy, frontend lint/typecheck/unit/build, 377 Playwright E2E cases, Docker build, Docker image `sha256:3fec51675af2dd16ac64648a7c80ea27dcfb0ef500a6010b5aeee8b1c2ec0ff2`, and Docker smoke. The Docker smoke used the Docker-assigned loopback port `http://127.0.0.1:32796`.
...
- Frontend unit tests: `3116 passed`
