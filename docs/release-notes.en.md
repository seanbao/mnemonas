...
Latest local full-validation snapshot: validation target `8012367e595a`; `GOTOOLCHAIN=local timeout 90m ./scripts/verify-changed.sh --base master` passed, covering diff whitespace, secret-leak scanning, workflow/YAML/script gates, the restore-report increment that uses raw restore-target matching to add explicit findings when the latest restore is complete but lacks matching read-only verification, when read-only verification predates restore completion, when it belongs to a different restore target, or when the read-only verification status cannot serve as current-target evidence, the login-page setup-safety hint increment for auth-disabled, share-enabled-without-auth, and anonymous WebDAV states, the Activity review cache update that immediately inserts updated records matching the current history filter, the reverse-proxy WebDAV verification documentation contract gate, the release-readiness checklist summary-scope gate, Release workflow structural gates, the hardening progress `make release-readiness` row-level validation-target gate, the `make release-readiness` entry-point baseline, `make check`, toolchain consistency, Go/Rust/frontend dependency security scans, example config validation, public-access templates, protobuf regeneration stability, Rust fmt/test/clippy, proto-gen fmt/test/clippy, frontend lint/typecheck/unit/build, 377 Playwright E2E cases, Docker build, Docker image `sha256:b9ea31d3facdc06df868f5b679dc914672f8a1dadd5b6bcd5c3f41a61c122494`, and Docker smoke. The Docker smoke used the Docker-assigned loopback port `http://127.0.0.1:32798`.
...
- Frontend unit tests: `3118 passed`
