...
Latest local full-validation snapshot: validation target `1e37ae700064`; `GOTOOLCHAIN=local timeout 90m ./scripts/verify-changed.sh --base master` passed, covering the prior roadmap, WebDAV documentation, CDC documentation boundary, Chinese visible-text, release-readiness baseline, community entry-point, documentation-contract, backup restore-drill guide contract, Go test-timeout gate, batch-restore preflight-failure disposition E2E, and release-note candidate-limit, public go-live backend-port TCP reachability, and timeout/gtimeout compatibility gate increments, plus the release-readiness gate requiring the release checklist and bilingual release notes to retain the `mnemonas-doctor --public-domain`, external-network `public-go-live-smoke.sh`, `cloud-firewall-checklist`, L1/L1+ candidate positioning, non-primary data-copy, and external-backup review entry points. It also covered `make check`, dependency security scans, example config validation, public-access templates, protobuf regeneration stability, Rust fmt/test/clippy, proto-gen fmt/test/clippy, frontend lint/typecheck/unit/build, 377 Playwright E2E cases, Docker build, Docker image `sha256:52cb9abaec3e9011ec4d864c757845dcf98813adc1e566221adf9f06c9b2eaba`, and Docker smoke. The Docker smoke used the Docker-assigned loopback port `http://127.0.0.1:32893`.
