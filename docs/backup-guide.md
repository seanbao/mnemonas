...
维护页和恢复报告只会把目标路径一致、校验时间不早于最近恢复完成时间，且最近恢复已成功完成的 `last_restore_verify` 关联到最近一次恢复；任务视图和恢复报告都会把该匹配结果写入 `last_matching_restore_verify`，不匹配时会提示最近恢复尚未完成匹配的只读校验。最近恢复仍在运行时，恢复报告会明确提示恢复未完成，且不会关联旧的只读校验结果。
