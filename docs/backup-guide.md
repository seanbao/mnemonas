...
缺失文件、校验和/大小/权限不匹配、额外常规文件或目录、缺失目录、目录权限漂移，以及已存在的 `.mnemonas-restore/config.toml` 与对照 manifest 不一致，都会作为警告返回。最近一次报告会持久化为 `last_restore_verify`，刷新维护页后仍可查看。维护页会在恢复成功后自动进入恢复后切换清单，并显示本次恢复的回滚清单。
