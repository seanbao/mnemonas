...
- `local`：把来源目录复制成带 v2 `manifest.json` 的本地快照。
...
- 备份管理器在 `<storage.root>/.mnemonas/backup/backup-state.lock` 上持有跨进程锁，直到管理器关闭。同一状态根目录同时只能有一个写入者；锁文件在正常关闭后会保留，是否被占用由操作系统文件锁决定。该文件不是过期锁标记，MnemoNAS 运行时不得删除。
- Unix 上的备份状态根目录必须由当前服务账号或 `root` 拥有，且不得允许 group/other 写入。其祖先目录必须由可信账号拥有，并且不得让其他本地账号替换该状态根目录；不满足此条件时，备份管理器不会初始化。
- 管理器保留已锁定状态根目录的文件系统身份，并在处理操作和状态写入前后验证原路径仍指向该目录。状态根目录被移动、删除或替换后，身份校验会立即隔离当前管理器，不会跟随新路径或向替换目录写入。如果当前操作的 `completed` 结果已经原子提交，该次操作保持完成，并返回“请检查状态目录并重启服务”的 warning；后续备份 API 返回 `503`。未完成原子提交的普通硬持久化失败返回 `500`。
- `local` 任务在 `<destination>/<job-id>/.mnemonas-target.lock` 上持有跨进程锁。备份、恢复预览、恢复演练、恢复、恢复后只读校验和保留检测均使用该锁，因此即使两个管理器使用不同状态根目录，也不会同时操作同一本地任务目录。Unix 上的任务目录必须由当前服务账号或 `root` 拥有，且不得允许 group/other 写入；祖先目录必须由可信账号拥有，可写祖先只有在设置 sticky bit 且能防止其他账号替换可信子目录时才会被接受。不满足这些条件时，本地操作会拒绝继续。
- FAT、exFAT 等不持久保存 Unix 所有者和 mode 的外置盘，必须用安全的 `uid`、`gid`、`dmask` 和 `fmask` 挂载，使任务目录对服务进程显示为可信所有者，且 group/other 不可写。例如，可按实际服务 UID/GID 设置 `uid=<mnemonas-uid>,gid=<mnemonas-gid>,dmask=0077,fmask=0177`。不具备可靠 Unix mode，且没有此类挂载约束的目标会被本地任务以 fail-closed 方式拒绝。
- v2 本地 manifest 要求来源、快照和恢复目标都提供稳定的 POSIX `rwx` 权限语义。原生 Windows 无法保真保存这些权限位，因此 MnemoNAS 会拒绝在原生 Windows 上执行 v2 本地备份、恢复预览、恢复、恢复演练、恢复校验和保留检测。
- 远端任务不创建本地目标锁。MnemoNAS 依赖 restic 的原生仓库锁协调指向同一仓库的进程。rclone 没有适用于所有 remote 的通用分布式互斥机制；如果多个任务或 MnemoNAS 实例指向同一 rclone 路径，必须用外部调度串行化备份、恢复和校验操作。
...
`local` 保留策略在成功备份状态提交后执行，始终保留当前快照。自动清理只处理 manifest 身份与快照树布局校验通过的 v2 快照；v1 快照会固定保留并产生处置提醒，不能阻塞 v2 快照按 `max_snapshots` 或 `max_age` 清理。已完成恢复历史仍引用的快照也会固定保留，直到对应恢复记录不再位于保留历史中。`max_snapshots = 0` 和 `max_age = "0"` 表示不启用对应维度的自动清理。

备份运行按以下顺序提交：

1. 在创建本地快照或启动远端命令前持久化 `running` 状态。
2. 对本地任务同步快照文件、manifest 和暂存目录，再将暂存目录重命名为最终快照并同步快照根目录。
3. 持久化包含快照路径、manifest 大小与摘要证据的成功运行状态。
4. 成功状态提交后再执行旧快照清理和保留策略检测。

如果 `running` 状态无法持久化，运行会失败，且不会创建快照或启动远端备份命令。如果成功状态在原子替换前持久化失败，运行返回失败，之前的 `last_successful_run` 保持不变；即使最终快照目录已经出现，没有绑定到成功状态的快照也不会用于预览、恢复或恢复演练。如果状态文件已经替换，但父目录同步结果不确定，运行保持 `completed`，保留现有快照并跳过后续清理；API 返回 `warning=true`、`warnings[]`、`message="backup completed with warnings"` 和 `Warning: 199 MnemoNAS "backup run completed with warnings"`。
...
本地快照目录名不是规范运行 ID，或 `snapshots/` 根目录出现非快照条目（只有 `.partial` 目录会被当作未完成快照跳过），或 manifest 缺失、manifest 路径包含符号链接、manifest 不是常规文件时，保留策略检测会失败并写入失败结果。身份字段有效的 v1 manifest 只供保留策略识别旧快照，不会被恢复或自动删除；应在确认不再需要后人工处置。

本地 v2 manifest 会分别登记普通文件与 `data/` 目录。目录清单精确绑定目录拓扑、空目录和 POSIX `rwx` 权限位；setuid、setgid、sticky bit 等特殊权限位会被拒绝，ACL、扩展属性和所有者身份不在该证据范围内。任务 ID、运行 ID、`created_at` 不匹配、不安全归档路径、重复或未排序路径、父目录缺失、负文件大小、无效权限位、无效 SHA-256、统计字段不一致、manifest 未登记的文件或目录、缺少已登记目录，或出现顶层异常目录时，校验也会失败；格式错误的 manifest 或布局不一致的快照会被拒绝。`config/` 是固定结构目录，不进入目录清单；恢复时始终以 `0700` 创建。

本地恢复预览、恢复和恢复演练只信任 `status.json` 中 `last_successful_run` 指向的快照，不会通过扫描 `snapshots/` 目录推断可恢复快照。持久化的快照路径和 manifest 路径必须与当前 `local.destination/<job-id>/snapshots/<run-id>/manifest.json` 一致；manifest 必须是无符号链接路径上的普通文件，其实际大小和摘要必须与成功运行时保存的证据一致，任务、运行和统计字段也必须匹配。v1 manifest 或缺少这些证据的旧状态不能作为恢复依据，需要先完成一次新的本地备份。
...
备份、恢复、恢复演练、只读校验和保留检测开始执行时会先写入 `running` 状态。服务启动时，如果状态文件中仍有上次进程退出前遗留的 `running` 记录，会将其标记为失败并写回状态文件。备份已提交、但后续保留清理、保留检测或结果持久化未完整完成时，运行保持 `completed` 并返回 warning，不会把已经提交的备份改写为整体失败。
...
`local` 恢复演练会先按受信 v2 manifest 核对快照文件和完整目录集合，再从 manifest 重建临时目录，而不是从快照当前目录树推断拓扑或权限。演练会校验目录拓扑、空目录、POSIX `rwx` 权限位，以及每个文件的大小、权限位和 SHA-256。`keep_artifact = true` 会保留临时恢复目录，便于人工抽查。默认不保留产物时，如果校验完成但临时恢复目录清理失败，演练结果保持 completed，同时返回 `warning=true`、`warnings[]`、`artifact_kept=true` 和 `restored_path`，便于在维护页继续处理残留产物。`restic` 恢复演练当前执行 `restic check`；`rclone` 恢复演练当前执行 `rclone check --one-way`，用于验证仓库或远端一致性。
...
`restore-preview` 不会写入目标目录，也不会写入恢复历史。它会复用恢复目标安全校验，并返回预计文件数、字节数、最多 10 个样例路径、`preflight_checks`、`warnings`、`cutover_checklist` 和 `rollback_checklist`。本地恢复预览会先按受信 manifest 检查快照布局，拒绝未登记的文件或目录、缺失目录、目录权限漂移和顶层异常目录。
...
`restore` 写入前会在服务端重新执行同一套预检，并先按受信 manifest 核对本地快照；预检或快照校验失败会拒绝恢复并写入失败恢复记录。`local` 恢复以 manifest 目录清单为真值，在目标根目录重建 `data/` 的精确拓扑、空目录和 POSIX `rwx` 权限位，再复制并立即校验文件，不读取快照目录当前权限来决定恢复结果。安装目标前还会按显式恢复布局执行一次完整树校验，拒绝额外或缺失条目、权限漂移以及目标文件系统上的大小写折叠冲突。`include_config = true` 时，配置文件会恢复到 `target_path/.mnemonas-restore/config.toml`，结构目录 `.mnemonas-restore/` 固定为 `0700`。

如果同一 manifest 同时包含可恢复配置，并且 `data/` 树已占用 `.mnemonas-restore` 文件或目录命名空间（含仅大小写不同的等价名称），则 `include_config = true` 的 `restore-preview` 和 `restore` 会拒绝继续，避免配置载荷覆盖用户数据。只恢复数据时，该名称不作为配置保留命名空间，`data/` 树可以按 manifest 正常恢复。

显式恢复使用与目标同级的私有暂存目录。Unix 上该目录及其祖先必须满足可信所有者和不可替换约束；系统会在外部命令或文件复制前记录暂存目录身份，并在创建后以及安装和清理边界复核该身份。暂存目录被移动、替换或经符号链接重定向时，恢复以不安全路径失败，不会安装或删除替换对象。
...
对于 `local` 任务，它会优先使用同一目标最近一次成功恢复所绑定的受信 v2 manifest；没有匹配恢复记录时，才回退到最近一次成功本地备份绑定的 manifest，并返回比较使用的 `snapshot_path` 和 `manifest_path`。文件和目录比较都以 manifest 为真值，不从快照的当前目录树重新推断基线。

恢复记录会在内部保存 manifest 大小和原始字节摘要绑定，供后续 `restore-verify` 选择同一次恢复的可信基线；只改变 JSON 空白或字段排列也会使该绑定失效。这些证据不属于公共 `RestoreResult` API 字段，API 客户端不应依赖或提交它们。旧恢复记录缺少内部证据时，可先使用当前受信 v2 备份重新恢复并校验；当前备份也是 v1 或缺少受信运行证据时，才需要先完成新的本地备份。

缺失文件、校验和/大小/权限不匹配、额外常规文件或目录、缺失目录、目录权限漂移，以及恢复记录要求配置证据时缺失 `.mnemonas-restore/config.toml`、配置内容不一致或结构目录不是精确 `0700`，都会作为警告返回。最近一次报告会持久化为 `last_restore_verify`，刷新维护页后仍可查看。维护页会在恢复成功后自动进入恢复后切换清单，并显示本次恢复的回滚清单；两组清单均可逐项确认，并显示本地确认进度。
...
恢复演练提醒会限频发送，并仅在通知通道成功返回后于任务视图记录 `last_restore_drill_reminder_at`。投递失败不会推进冷却时间；提醒已送达但标记持久化失败时，下次调度会重试，因此可能重复投递。可使用 Webhook、Telegram、企业微信、钉钉或 SMTP 邮件。Webhook 启用方式：
...
自定义 `backup.Notifier` 的 `NotifyBackupEvent` 是同步调用。备份管理器为每次调用提供 10 秒 deadline，并在管理器关闭时取消 context；实现必须监听 `ctx.Done()`，并在 context 取消或超时后及时返回。忽略 context 的阻塞实现可能卡住当前备份操作或服务关闭。内置 SMTP 传输还设置 30 秒默认超时，并优先遵循更早到期的上游 context deadline；因此备份事件仍受 10 秒通知预算限制。
