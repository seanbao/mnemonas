...
- `local`: copy a source directory into a local snapshot with a v2 `manifest.json`.
...
- `restic` and `rclone` jobs do not build shell command strings; `command` must be a bare executable name or absolute path without whitespace or control characters, and `extra_args`, `exclude`, and `retention_policy` must not contain control characters. `extra_args` are appended to backup commands but cannot override job identity. Rclone currently accepts only `--fast-list`; restore commands do not reuse backup-specific extra args.
- `restic.repository` accepts only an absolute local path outside `source` and `storage.root`, or an explicit `rest:http://` or `rest:https://` REST server URL. Repository forms that depend on cloud SDK, SSH, or rclone credential discovery are not accepted.
- `restic.password_file` and `rclone.config_file` are required for their respective job types. Each file must be a non-symlink regular file outside `source` and `storage.root`, its existing path components must not be symlinks, and its size must not exceed 4 MiB.
- `rclone.remote` must reference a named remote in `config_file` whose section declares `type`; for example, `cloud:mnemonas/current` references `[cloud]`. The file must be a static, self-contained plaintext config: it cannot enable `env_auth`, contain `${...}` expansion, or use a non-empty option whose name contains `_file`, `_path`, `command`, `agent`, `ssh`, or `token` to depend on an external file, command, agent, SSH identity, or refreshable token. The current evidence model does not support encrypted rclone configs or automatic token write-back.
- Before a remote command starts, the server creates a private mode-0700 directory under the resolved system temporary directory and copies the credential into a mode-0600 snapshot. The directory must be outside `source` and `storage.root`, and the command reads only the snapshot. Child processes do not inherit cloud-provider credentials, proxy settings, SSH agents, or `RESTIC_*` and `RCLONE_*` variables that can override the explicit job identity; repositories, remotes, and credential files come only from the validated job configuration.
- Job views, run results, restore or preview results, restore reports, and batch restore results redact embedded userinfo, tokens, passwords, secrets, and key parameters in target paths, remote target fields, and API-visible backup error, warning, or restore-report findings text. `_` and `-` separators in parameter names are also recognized when encoded as `%5F` or `%2D`.
...
Local retention runs after the successful backup state is committed and always keeps the current snapshot. Automatic pruning applies only to v2 snapshots whose manifest identity and snapshot tree layout validate. V1 snapshots remain pinned with a disposition warning and do not block v2 snapshots from being pruned by `max_snapshots` or `max_age`. Snapshots referenced by completed restore history also remain pinned until the corresponding restore record leaves retained history. `max_snapshots = 0` and `max_age = "0"` disable the corresponding pruning dimension.
...
A local snapshot directory with a non-canonical run ID, a non-snapshot entry in the `snapshots/` root (only `.partial` directories are skipped as incomplete snapshots), a missing manifest, a manifest path that contains a symlink, or a manifest that is not a regular file fails the retention check and persists a failed result. A v1 manifest with valid identity fields is recognized only to pin the legacy snapshot; it cannot be restored or automatically removed and requires manual disposition after it is no longer needed.

A local v2 manifest records regular files and `data/` directories separately. The directory list binds the exact topology, empty directories, and POSIX `rwx` permission bits; setuid, setgid, sticky, and other special permission bits are rejected, while ACLs, extended attributes, and ownership are outside this evidence scope. Validation also fails on a mismatched job ID, run ID, or `created_at`, unsafe archive path, duplicate or unsorted path, missing parent directory, negative file size, invalid mode, invalid SHA-256, inconsistent summary fields, unmanifested file or directory, missing manifested directory, or unexpected top-level directory. A malformed manifest or snapshot with an inconsistent layout is rejected. The structural `config/` directory is not part of the directory list and is always created with mode `0700` during restore.

Local restore previews, restores, and restore drills trust only the snapshot referenced by `last_successful_run` in `status.json`; they do not infer a restorable snapshot by scanning the `snapshots/` directory. The persisted snapshot and manifest paths must match the current `local.destination/<job-id>/snapshots/<run-id>/manifest.json` location. The manifest must be a regular file reached without symlinks, its actual size and digest must match the evidence saved by the successful run, and its job, run, and summary fields must agree. A v1 manifest or older state without this evidence cannot be used for restore; a new successful local backup is required.
...
For `local`, the restore drill first checks the snapshot files and complete directory set against the trusted v2 manifest, then rebuilds the temporary directory from the manifest instead of inferring topology or permissions from the snapshot's current directory tree. The drill verifies directory topology, empty directories, POSIX `rwx` permission bits, and every file's size, mode, and SHA-256. Set `keep_artifact = true` to retain the restored directory for manual inspection. With the default non-retained artifact behavior, if verification completes but temporary restore-directory cleanup fails, the drill remains completed and returns `warning=true`, `warnings[]`, `artifact_kept=true`, and `restored_path` so the Maintenance page can surface the retained artifact. For `restic`, the drill currently runs `restic check`; for `rclone`, it runs `rclone check --one-way` to verify remote consistency.
...
`restore` reruns the same server-side preflight before writing and checks the local snapshot against the trusted manifest; failed preflight or snapshot verification rejects the restore and is persisted with the failed restore record. Local restore treats the manifest directory list as authoritative, reconstructs the exact `data/` topology, empty directories, and POSIX `rwx` permission bits at the target root, then copies and immediately verifies files. It does not read current snapshot-directory permissions to determine the restored result. Before installation, a final explicit-layout tree check rejects extra or missing entries, permission drift, and case-folding collisions on the target filesystem. With `include_config = true`, the config file is restored to `target_path/.mnemonas-restore/config.toml`, and the structural `.mnemonas-restore/` directory uses fixed mode `0700`.

If the same manifest includes a restorable config and its `data/` tree already occupies the `.mnemonas-restore` file or directory namespace, including a case-only equivalent, `restore-preview` and `restore` reject `include_config = true` so the config payload cannot overwrite user data. A data-only restore does not reserve that namespace for config.

Explicit restore uses a private staging directory adjacent to the target. On Unix, that directory and its ancestors must satisfy trusted-owner and non-replaceability constraints. MnemoNAS records the staging-directory identity before external commands or file copying and rechecks it after creation and at installation and cleanup boundaries. Moving, replacing, or symlink-redirecting the staging path fails the restore as unsafe without installing or deleting the replacement object.
...
For `local` jobs, it first uses the trusted v2 manifest bound to the latest successful restore for the same target. When no matching restore record exists, it falls back to the manifest bound to the latest successful local backup and returns the comparison `snapshot_path` and `manifest_path`. Both file and directory comparisons treat the manifest as authoritative instead of deriving a baseline from the snapshot's current directory tree.

Restore records retain an internal manifest size and raw-byte digest binding so later `restore-verify` operations can select the trusted baseline for that restore; changing only JSON whitespace or field order invalidates that binding. This evidence is not part of the public `RestoreResult` API fields, and API clients must not depend on or submit it. When an older restore record lacks internal evidence, the current trusted v2 backup can first be restored and verified again. A new local backup is required first only when the current backup is also v1 or lacks trusted run evidence.

It warns about missing files, checksum/size/mode mismatches, extra regular files or directories, missing directories, directory permission drift, and, when the matching restore record requires config evidence, a missing or mismatched `.mnemonas-restore/config.toml` or a structural directory whose mode is not exactly `0700`. The latest report is persisted as `last_restore_verify` so it remains visible after refreshing the Maintenance page. The Maintenance page enters the post-restore cutover checklist automatically after a successful restore and also displays the rollback checklist for that restore. Both checklists support per-item confirmation and show local confirmation progress.
