...
Local restore previews, restores, and restore drills trust only the snapshot referenced by `last_successful_run` in `status.json`; they do not infer a restorable snapshot by scanning the `snapshots/` directory. The persisted snapshot and manifest paths must match the current `local.destination/<job-id>/snapshots/<run-id>/manifest.json` location. The manifest must be a regular file reached without symlinks, its actual size and digest must match the evidence saved by the successful run, and its job, run, and summary fields must agree. Older state without this evidence cannot be used for restore; a new successful local backup is required.
...
It warns about missing files, checksum/size/mode mismatches, extra regular files or directories, missing directories, directory permission drift, or mismatched restored `.mnemonas-restore/config.toml` contents when that file exists. The latest report is persisted as `last_restore_verify` so it remains visible after refreshing the Maintenance page. The Maintenance page enters the post-restore cutover checklist automatically after a successful restore and also displays the rollback checklist for that restore. Both checklists support per-item confirmation and show local confirmation progress.
