...
| `412` | Download-resumption identity or another request precondition not satisfied |
| `413` | File too large |
| `429` | Rate limited |
...
| `HEAD` | `/api/v1/download/{path}` | Return download headers for a current file or historical version without a response body |
...
`GET /api/v1/download/{path}` returns file bytes by default. `HEAD /api/v1/download/{path}` performs the same path, authorization, and snapshot-open checks for current files and historical versions and returns the corresponding headers without a response body; ZIP archives do not support `HEAD`. Supported query parameters:
...
Current-file `GET` and `HEAD` responses include `X-MnemoNAS-Download-Identity` when the platform can produce an object identity. The value is the `deleteIdentityToken` derived from the same opened regular-file handle. It is an opaque object-and-metadata identity, not a content hash, and producing the header does not read the complete file. FIFOs, Unix sockets, device nodes, and other non-regular files are rejected without blocking request handling while opening the target.

To resume a current-file download, a client retains the identity from the initial response and sends exactly one `X-MnemoNAS-If-Download-Identity` header on a later request. `Range` accompanies only the `GET` that appends bytes; `HEAD` ignores `Range` and returns headers for the complete representation. The server first opens the regular-file snapshot used by that request and then compares the request value byte-for-byte and case-sensitively with that snapshot's download identity. A matching Range `GET` continues through the existing `http.ServeContent` behavior, including `206 Partial Content` and `Content-Range`. A mismatch, repeated request header, or a platform that cannot produce an identity returns `412 Precondition Failed` with `Content-Length: 0` and no response body. When available, `X-MnemoNAS-Download-Identity` on the `412` response is the current value from the snapshot opened for that request; the client must discard its stale partial file or restart the download.

The weak ETag remains a cache revalidation mechanism; object continuity for resumed downloads is constrained separately by `X-MnemoNAS-Download-Identity` and `X-MnemoNAS-If-Download-Identity`. At the end of a GET, the server revalidates the opened file identity before committing a bounded final segment. If an out-of-band writer changes the same file during streaming, the response terminates below its declared `Content-Length`; a client must not commit that response as a complete file.
...
- Current-file and historical-version GET downloads support Range requests; HEAD ignores Range, and ZIP archive downloads do not guarantee Range support.
