...
- `local` jobs create snapshots under `destination/<job-id>/snapshots/<run-id>/`. The destination must be outside `storage.root` and the backup source, and path components must not cross boundaries through symlinks. The v2 manifest records the exact `data/` directory topology, empty directories, and POSIX `rwx` permission bits, and rejects setuid, setgid, sticky, and other special permission bits; ACLs, extended attributes, and ownership are outside this evidence scope. The structural `config/` directory is not part of the directory list and uses mode `0700` during restore.
...
- The backup state root is fixed at `<storage.root>/.mnemonas/backup`. The backup manager holds `backup-state.lock` for its entire lifetime, allowing only one writer for a state root. The lock file remains after shutdown and does not indicate a stale lock; it must not be removed while MnemoNAS is running.
- On Unix, the backup state root must be owned by the effective service account or `root` and must not be writable by group or other users. Its ancestors must have trusted owners and must not allow another local account to replace the state root. The backup manager does not initialize when it cannot acquire the state lock safely.
- The backup manager retains the identity of the locked state root and rechecks the original path at operation and state-write boundaries. Moving, removing, or replacing the path immediately quarantines the current manager, which does not write into the replacement directory. An operation's atomically committed `completed` result remains completed and returns a warning that instructs the operator to inspect the state directory and restart the service; later backup API requests return `503`. An ordinary hard persistence failure before atomic commit returns `500`.
- A `local` job holds `<destination>/<job-id>/.mnemonas-target.lock` during backup, restore preview, restore drill, restore, post-restore read-only verification, and retention-check operations, preventing different processes from operating on the same local job directory concurrently. On Unix, the job directory must be owned by the effective service account or `root` and must not be writable by group or other users. Its ancestors must have trusted owners; a writable ancestor is accepted only when the sticky bit is set and prevents another account from replacing a trusted child directory. Local operations fail closed when these conditions are not met.
- External disks using FAT, exFAT, or another filesystem that does not persist Unix ownership and mode require secure mount mappings. Set `uid=<mnemonas-uid>,gid=<mnemonas-gid>,dmask=0077,fmask=0177` to the actual service UID/GID so the job directory appears to have a trusted owner and is not writable by group or other users. A target without reliable Unix mode or equivalent mount constraints is rejected by `local` jobs.
...
- Remote jobs do not create a local target lock. Restic jobs rely on restic's native repository locking. Rclone has no generic distributed mutex for every remote, so external scheduling must serialize backup, restore, and verification operations when multiple jobs or instances share the same rclone path.
- Backup runs persist the `running` state before creating a local snapshot or starting a remote command. If that state cannot be saved, the run fails without backup-target side effects.
- Local snapshots sync their content, manifest, and staging directory before publishing the final directory and syncing the snapshot root. Old snapshots are pruned and retention is checked only after the successful state with manifest evidence is committed.
- If successful-state persistence fails before atomic replacement, the previous `last_successful_run` remains unchanged and the unbound final snapshot is not used for restore. If the backup is committed but later directory sync, cleanup, retention checking, or result persistence is incomplete, the run remains `completed`, and the API reports the incomplete step through `warning=true`, `warnings[]`, and the HTTP `Warning` header.
- Local restore, preview, and restore-drill operations accept only the v2 manifest path, size, and digest evidence stored in `status.json.last_successful_run`; they do not scan unbound snapshot directories. Restore, restore drill, and post-restore read-only verification treat the trusted manifest as authoritative for directory topology and permissions instead of deriving them from the snapshot's current directory tree. A final explicit-target tree check before installation rejects extra or missing entries, permission drift, and case-folding collisions. A new successful local backup is required for a v1 manifest or when evidence is absent.
- Restore records retain the manifest size and raw-byte digest as internal evidence so later read-only verification can select the trusted baseline for that restore; changing only JSON whitespace or field order invalidates the binding. This evidence is not returned as public `RestoreResult` API fields.
...
- These v2 directory-list semantics apply only to `local`; `restic` and `rclone` continue to use their respective backend snapshot, copy, and verification semantics.
...
- `restore_drill_stale_after` controls missing or stale restore-drill reminders. Blank, `"0"`, or omitted values are treated as 30 days at runtime. The reminder cooldown marker is persisted only after a notification channel returns success; delivery or marker-persistence failure is retried by a later scheduler pass.
- Local retention cleanup always keeps the current snapshot and snapshots still referenced by completed restore history. Automatic pruning applies only to v2 snapshots whose manifest identity and snapshot tree layout validate. V1 snapshots remain pinned with a manual-disposition warning and do not participate in v2 count or age calculations. Actual retention for `restic` and `rclone` is managed by the external tool or cloud lifecycle rules, and `retention_policy` records that the deployment-side policy has been confirmed.
...
The backup manager invokes a custom `backup.Notifier` synchronously. Every `NotifyBackupEvent` call carries a 10-second deadline and is canceled when the manager shuts down. A custom implementation must observe `ctx.Done()` and return promptly after context cancellation or deadline expiry; otherwise it can block a backup operation or service shutdown. The built-in SMTP transport has a 30-second default timeout and honors an earlier upstream deadline, so backup events remain subject to the 10-second notification budget.
