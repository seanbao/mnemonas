...
- When `[alerts] enabled = true` and Webhook, Telegram, WeCom, or SMTP email is configured, missing devices, SMART failures, high temperature, serial mismatch, or unavailable SMART data send a `disk_health` event. Event details use aggregate counts and do not include device names, full device paths, serial numbers, or warning text.
