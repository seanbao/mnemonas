...
- Before saving, the Web settings page summarizes added, changed, and removed directory access rules by comparing the saved rules with the current draft, and shows a draft coverage summary for rule count, read/write principals, write-enabled paths, and attention items such as root-path or broad role grants. User matrices and unsaved-rule previews can copy directory-access review records and keep current-browser recent review history. The page uses a structured rule editor; enter MnemoNAS logical paths directly in the path field, including spaces or literal double quotes, without manual line quoting.
