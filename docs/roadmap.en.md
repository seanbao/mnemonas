...
| Capability | Gap | Suggested implementation | Acceptance criteria |
| --- | --- | --- | --- |
| Folder-level permissions | Basic directory ACLs, groups, single-user effective access checks, unsaved-rule previews, per-path user matrix views, family/small-team policy presets, pre-save rule-change review, directory-access coverage summary, related-share impact checks, and copyable directory-access review records exist; more detailed bulk-application drilldown and persisted review history are still missing | Add more detailed bulk-application drilldown and persisted review history | Web, API, WebDAV, and shares use the same permission decision, and admins can review effective access |
