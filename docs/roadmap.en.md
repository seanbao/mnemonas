...
| Notifications | Webhook, Telegram, and SMTP email cover disk-space, backup failures or warnings, explicit restore failures or warnings, post-restore read-only verification failures or warnings, restore-drill reminders, retention-check failures or warnings, disk-health anomalies, Scrub anomalies, login rate limits, and Web/API quota denials; Web/API provides a saved-config `alert_test` test entry point | Support WeCom-style channels and more event sources such as permission changes | Disk-full, backup-failed, restore-failed or warning, Scrub-failed, login-rate-limited, and quota-exceeded events can notify users; administrators can send a test alert to verify saved channels |
| User quotas | User-level quotas, directory quotas, dynamic usage, directory quota usage summaries, Web/API upload/copy/move/trash-restore user-quota enforcement for writes into `home_dir`, WebDAV PUT/COPY/MOVE user-quota enforcement for writes into `home_dir`, directory-quota enforcement for version restores, and Web/API quota-denial alerts exist | Add richer admin views, trends, and permission-aware quota views | Uploads, copies, moves, trash restores, version restores, and WebDAV writes honor the applicable user or directory quotas with clear errors and notifications |
...
| Activity review | Activity list, statistics overview, high-risk summary, concentrated-window review scoped to the high-risk group, time-range, path/directory, share/high-risk group, action-type, and user filtering, and administrator-only clear action exist | Add more detailed bulk-change drilldown and review views | Key activity can be reviewed by time range, path/directory, user, and action type to trace accidental deletes, moves, or shares |
