...
| Android client | A cross-platform Flutter project now covers connection, revision/CAS session updates, core file operations, bounded filename search, version history and safe administrator restore, Trash workflows, durable foreground upload and download, a state-based transfer center, server-authoritative offsets, safe SAF upload selection, and durable-task pausing with transient-task cancellation on background entry; release builds have a fail-closed signing gate, while an API 35 emulator and API 36 physical device cover basic back behavior, session recovery, and a Debug update; Android is the first usable target, with no usable version available | Replace the transfer ledger with transactional revision/CAS and fencing-token ownership, then connect background execution and notifications through Android 14+ UIDT jobs and an Android 7–13 WorkManager fallback; establish production-key custody and monotonic release versioning, then complete HTTPS, foreground/background pause and recovery, network-loss, permission-denial, and API 24/33/34 validation | Produce a candidate signed with controlled material and verified on physical devices, with reproducible evidence for large upload/download, process-termination recovery, and accidental-deletion recovery |
