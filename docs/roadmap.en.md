...
| Android client | A cross-platform Flutter project now covers connection, revision/CAS session updates, core file operations, bounded filename search, Trash workflows, durable foreground upload and download, server-authoritative offsets, and safe SAF upload selection; Android is the first usable target, with no usable version available | Implement the Android native background executor and cross-process task leases, then validate installation and upgrade, session recovery, HTTPS, foreground/background transitions, network loss, permission denial, and release signing | Produce a signed candidate verified on physical devices, with reproducible evidence for large upload/download, process-termination recovery, and accidental-deletion recovery |
