...
| Deployment reliability | Docker and systemd paths, post-build loopback Docker container smoke tests, release install/uninstall safety tests, and diff-aware validation exist | Continue formalizing upgrade, rollback, and config migration | Release install, upgrade, uninstall, data retention, and Docker image health startup are covered by automated tests |
