...
| Deployment reliability | Docker and systemd paths, a systemd successful-upgrade test preserving existing config and data, post-build loopback Docker container smoke tests, release install/uninstall safety tests, and diff-aware validation exist | Continue formalizing real rollback, config-migration, and cross-version drill flows | Release install, upgrade, uninstall, data retention, and Docker image health startup are covered by automated tests |
...
- Restore flow: safe-directory local snapshot restore, safe-directory restic/rclone remote preview and restore, restore preflight, failed-preflight blocking, post-restore read-only verification, latest-restore recheck, cutover/rollback checklists, and restore-summary export.
