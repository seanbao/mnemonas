...
| Activity review | Activity list, statistics overview, high-risk summary, concentrated-window review scoped to the high-risk group, current-page review details, current-page review disposition checklist, time-range, path/directory, share/high-risk group, action-type, and user filtering, a share-list handoff to matching path-scoped share activity, and administrator-only clear action exist | Add cross-page review history and persisted bulk-change disposition records | Key activity can be reviewed by time range, path/directory, user, and action type to trace accidental deletes, moves, or shares |
