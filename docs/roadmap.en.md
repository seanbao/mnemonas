...
| Android client | A cross-platform Flutter project now covers connection, revision/CAS session updates, core file operations, bounded filename search, version history and safe administrator restore, Trash workflows, durable foreground upload and download, a state-based transfer center, server-authoritative offsets, safe SAF upload selection, and durable-task pausing with transient-task cancellation on background entry; Android is the first usable target, with no usable version available | Replace the transfer ledger with transactional revision/CAS and fencing-token ownership, then connect background execution and notifications through Android 14+ UIDT jobs and an Android 7–13 WorkManager fallback before validating installation and upgrade, session recovery, HTTPS, foreground/background pause and recovery, network loss, permission denial, and release signing | Produce a signed candidate verified on physical devices, with reproducible evidence for large upload/download, process-termination recovery, and accidental-deletion recovery |
