...
`make client-check` 是 Flutter 客户端的主机侧门禁，要求仓库固定的 Flutter 版本和 JDK 17。该目标依次检查 Android 备份策略、Dart 格式、静态分析、全部 Flutter 测试，并构建 debug APK。测试覆盖会话与上下文隔离、文件和回收站流程、目录新鲜度与文件修改单飞、版本历史契约和管理员恢复的待确认语义、下载身份与断点恢复、持久传输账本、后台切换时的持久任务暂停与临时任务取消、Android 平台通道参数、进度和取消契约，以及主要页面状态。

主机侧门禁不能替代设备验收。涉及 Storage Access Framework、Activity 销毁、系统取消、权限拒绝、不同 DocumentsProvider、大文件、断网、进程终止、前后台切换或升级的变更，还应在 Android 模拟器和真实设备上验证。debug APK 只用于开发验证，不构成签名、升级兼容或可发布版本证据。
