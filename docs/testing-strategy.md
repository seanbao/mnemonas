...
- 认证 store、默认管理员创建、登录、token refresh、强制密码变更生命周期和初始密码文件删除。
- 配置文件加载与校验。
...
`scripts/webdav-client-smoke.sh` 用于已经运行的服务，提供独立 curl 协议 smoke，并覆盖 URL 编码空格路径读写；`WEBDAV_URL` 必须是不包含空白、query、fragment、内嵌凭据、反斜杠、编码斜杠或编码反斜杠，也不包含 `.`/`..` 路径段的 HTTP(S) WebDAV 根 URL，需要认证时通过 `MNEMONAS_WEBDAV_USERNAME` 和 `MNEMONAS_WEBDAV_PASSWORD` 传入凭据。每次 curl 请求默认使用 `CURL_CONNECT_TIMEOUT=10` 和 `CURL_MAX_TIME=30`，高延迟网络可通过环境变量调大。
`scripts/backup-restore-drill-smoke.sh` 也用于已经运行的服务，会按显式备份任务 ID 读取任务、触发立即备份、执行保留策略检查、运行恢复演练并下载恢复报告。脚本不会创建或删除备份任务；`MNEMONAS_API_URL` 必须是不含空白、query、fragment、内嵌凭据、反斜杠、编码斜杠/反斜杠、空路径段或点段的 HTTP(S) API 根 URL，`MNEMONAS_BACKUP_JOB_ID` 只能使用安全任务 ID。需要认证时通过 `MNEMONAS_COOKIE_FILE` 传入 curl cookie 文件；每次请求默认使用 `CURL_CONNECT_TIMEOUT=10` 和 `CURL_MAX_TIME=600`。
...
- Flutter 客户端的 Android 备份策略、格式、静态分析、单元与组件测试，以及 debug APK 构建。
...
Codecov 状态仅用于趋势展示和变更结果注释，不能作为唯一的阻塞质量门禁。
...
- Go、Rust dataplane、`tools/proto-gen`、protobuf、Web UI、Flutter 客户端、Playwright E2E、Docker、文档、GitHub Actions 工作流和 shell 脚本变更。
- Web UI 变更运行带阈值的 Vitest 覆盖率测试；`web/src` 下的 TSX 和 CSS 变更还会运行前端质量 E2E，检查可访问性、响应式布局和交互完整性。
...
`make client-check` 是 Flutter 客户端的主机侧门禁，要求仓库固定的 Flutter 版本和 JDK 17。该目标依次检查 Android 备份与应用身份策略、Dart 格式、静态分析、全部 Flutter 测试，并构建 debug APK。测试覆盖会话与上下文隔离、文件和回收站流程、目录新鲜度与文件修改单飞、版本历史契约和管理员恢复的待确认语义、下载身份与断点恢复、持久传输账本、后台切换时的持久任务暂停与临时任务取消、Android 平台通道参数、进度和取消契约，以及主要页面状态。

`make client-android-release-signing-check` 使用临时测试密钥验证 release 构建失败关闭策略。该门禁要求缺失或不可信签名配置被拒绝，并对生成的 APK 与 AAB 执行签名身份检查；CI 运行同一脚本。测试密钥、Debug 升级和临时签名产物都不能替代受控正式密钥及正式候选包验证。

主机侧门禁不能替代设备验收。涉及 Storage Access Framework、Activity 销毁、系统取消、权限拒绝、不同 DocumentsProvider、大文件、断网、进程终止、前后台切换或升级的变更，还应在 Android 模拟器和真实设备上验证。Debug 包的同签名覆盖升级可以证明该测试变体的数据保留与基本升级路径，但不构成 release 签名、正式升级兼容或可发布版本证据。
...
Vitest 覆盖率不统计 `web/e2e` 下的 Playwright helper。真实浏览器测试负责覆盖这些 helper，因此它们不计入单元测试覆盖率分母。前端覆盖率阈值统一在 `web/vitest.config.ts` 中管理。
...
账号进入强制密码变更门禁时，还必须设置与当前密码不同、包含 8 至 72 个 UTF-8 字节且不全为空白字符的 `E2E_PASSWORD_CHANGE_TO`。认证 setup 会完成修改并重新登录；该门禁不会按认证缺失处理，也不会因 `MNEMONAS_E2E_ALLOW_AUTH_SKIP=1` 而被跳过。
初始密码成功更换后，服务会删除初始密码文件；后续运行应通过 `E2E_PASSWORD` 提供替换后的密码。
