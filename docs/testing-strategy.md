...
`make client-check` 是 Flutter 客户端的主机侧门禁，要求仓库固定的 Flutter 版本和 JDK 17。该目标依次检查 Android 备份策略、Dart 格式、静态分析、全部 Flutter 测试，并构建 debug APK。测试覆盖会话与上下文隔离、文件和回收站流程、下载身份与断点恢复、持久传输账本、Android 平台通道参数、进度和取消契约，以及主要页面状态。
