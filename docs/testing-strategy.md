...
`scripts/run-fault-injection-isolated.sh` 会启动隔离后端，并调用 `scripts/fault-injection-test.sh` 执行破坏性验证。底层 runner 专门验证崩溃恢复、并发写冲突、版本恢复、对象损坏和元数据损坏处理。它会杀死并重启目标 `nasd`，并可直接改写内部数据文件，所以没有显式目标信息时会拒绝运行。
...
- `scripts/run-fault-injection-isolated.sh` 只接受 `/tmp` 或当前 checkout 下的根目录，并且 Web 与 dataplane 地址必须是 loopback
...
- 会被读取或改写的 `OBJECTS_DIR`、`INDEX_DB` 和可选 `NASD_PID_FILE` 必须位于 `STORAGE_ROOT` 下
...
├── run-fault-injection-isolated.sh # 隔离故障注入包装入口
...
默认 Playwright 配置会自动启动隔离的后端和前端测试服务器；默认后端端口为 `18180`，默认前端端口为 `14173`。隔离后端使用 2 小时 Access Token 有效期和 168 小时 Refresh Token 有效期，以降低长时间并行测试中的共享 storageState 过期风险。`MNEMONAS_E2E_BACKEND_URL` 和 `MNEMONAS_E2E_FRONTEND_URL` 可调整隔离服务器地址或端口。本地已有服务只有在设置 `MNEMONAS_E2E_REUSE_EXISTING=1` 时才会被复用，调试已有环境时还应显式设置 `E2E_PASSWORD`。
