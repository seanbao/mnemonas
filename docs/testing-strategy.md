...
`scripts/webdav-client-smoke.sh` 用于已经运行的服务，提供独立 curl 协议 smoke；需要认证时通过 `MNEMONAS_WEBDAV_USERNAME` 和 `MNEMONAS_WEBDAV_PASSWORD` 传入凭据。
...
| Standalone WebDAV smoke | curl `OPTIONS`、`MKCOL`、`PUT`、`PROPFIND`、`GET`、`HEAD`、`COPY`、`MOVE`、`DELETE` | 手动指定已运行服务 |
