...
The full `secrets.json` also contains runtime signing secrets and should not be copied into support requests, chats, or logs. Keep it as a regular file with private permissions.
...
- [ ] The initial administrator password has been changed, and the `initial-password.txt` path is absent with no symlink or non-regular file left behind.
...
- [ ] If WebDAV keeps global Basic Auth with a generated password, `<storage.root>/secrets.json` exists, is not a symlink, is a regular file, and has private permissions.
