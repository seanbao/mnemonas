...
The `--public-domain` check first lowercases the domain and removes a single FQDN trailing dot, then verifies public HTTPS health, HTTP redirects to HTTPS on the same public domain, certificate hostname, remaining certificate validity, public-deployment authentication settings, administrator-account redundancy, share-link base URL, public-share API response cache boundaries, direct backend exposure, and dataplane port exposure, then prints the manual cloud-firewall review item. Certificate inspection requires `openssl` on the server. Public authentication checks require `auth.enabled = true`, `security.allow_unsafe_no_auth = false`, and authenticated WebDAV when WebDAV is enabled; if global Basic Auth remains in use, explicit common placeholder passwords or passwords shorter than 16 characters produce a warning. If generated Basic Auth is used, `secrets.json` must exist, must not be a symlink, must not pass through symlink components, must be a regular file, and should keep private permissions. Administrator redundancy is checked from `auth.users_file`, or `$STORAGE_ROOT/.mnemonas/users.json` when it is unset. The users file must be a valid list, each user needs non-empty unique `id` and `username` values, valid `role` and `disabled` fields, and enabled administrators must have a bcrypt-format `password_hash`. The users file and its directory must not be symlinks and must not pass through symlink components; the public check also reports whether the users file and its directory still allow group or other-user access. A missing file, JSON or structure validation failure, unusable administrator password hash, or zero enabled administrators fails the check; one usable enabled administrator produces a warning; two or more usable enabled administrators pass. The initial password check uses `initial-password.txt` in the same user-data directory, so a custom `auth.users_file` moves the checked password file location with it; in public deployments, a symlink, symlink component, or non-regular file at that path also fails the check. When sharing is enabled, `share.base_url` should use HTTPS on the default port, must not contain userinfo, query strings, or fragments, and must use a valid host name; HTTP, a non-443 port, userinfo, query strings, fragments, or an invalid host name fails the check, while empty values, a different host, or a path already ending in `/s` produce a manual review warning. `share.base_url` should be the site origin or the base path before `/s`; otherwise generated share URLs contain a duplicated `/s/s` route. The public-share API response check uses a reserved probe ID and verifies only that the missing-share JSON response that reaches MnemoNAS includes `Cache-Control: private, no-cache`, `Vary: Cookie`, `X-Content-Type-Options: nosniff`, and `Referrer-Policy: no-referrer`; it does not read real share IDs, passwords, or cookie values. If the probe returns `401`, `403`, or a redirect at the reverse proxy or access-control layer, the check reports a routing warning and skips JSON header validation; deployments that intend to expose public share links should route `/api/v1/public/shares/` through to MnemoNAS. It also reports detectable renewal paths: Nginx/certbot deployments should pass `sudo certbot renew --dry-run`, and Caddy deployments should have clean ACME logs from `sudo journalctl -u caddy --since '24 hours ago'`.

For public deployments, `mnemonas-doctor --public-domain` checks `auth.access_token_ttl` and `auth.refresh_token_ttl`. Access tokens longer than `1h` or refresh tokens longer than `720h` (30 days) produce warnings; empty, `0`, or negative values fail.

When sharing is enabled, `mnemonas-doctor --public-domain` also checks `share.default_expires_in` and `share.default_max_access`. No default expiry, values longer than `720h` (30 days), or `0` default access limits produce warnings; negative expiry or negative default access limits fail. For family public sharing, keep the default at `168h` (7 days) or another explicit expiry no longer than 30 days, and set an explicit default access limit such as `20`.
...
The full `secrets.json` also contains runtime signing secrets and should not be copied into support requests, chats, or logs. Keep it as a regular file with private permissions, and keep its path free of symlink components.
...
- [ ] The initial administrator password has been changed, and the `initial-password.txt` path is absent with no symlink, symlink component, or non-regular file left behind.
...
- [ ] `/etc/mnemonas/config.toml` is a private regular file and does not pass through symlink components.
...
- [ ] If WebDAV keeps global Basic Auth with a generated password, `<storage.root>/secrets.json` exists, is not a symlink, does not pass through symlink components, is a regular file, and has private permissions.
- [ ] If public sharing is enabled, `share.default_expires_in` is not empty or `0`, is at most `720h`, `share.default_max_access` is greater than `0`, and the `mnemonas-doctor --public-domain` public-share probe reaches MnemoNAS with a passing JSON response boundary check.
