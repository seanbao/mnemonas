...
- 文件页面会按当前策略明确区分“移入回收站”和“永久删除”，并在策略未知时停用删除而不影响浏览。文件列表为实际条目返回对象身份令牌；删除确认通过 `POST /api/v1/files-delete-intents` 提交所选条目的观察身份，并原子取得完整策略与目标树令牌。同一路径在点选后被新文件或目录替换时，即使类型、大小和修改时间相同，服务端也会拒绝旧身份。目标令牌现采用 v3 分层 SHA-256 Merkle 表示：最终令牌绑定规范化根路径与完整路径层级；每个节点摘要绑定文件或目录的域分隔标记、完整权限模式（`mode`）、大小、纳秒级修改时间、对象身份和快照中的内容哈希字段，普通文件的该字段承载实际内容摘要；目录节点按子项名称排序，并依次组合子项名称与摘要。删除确认会在同一存储读锁内对每个目标树执行一次严格遍历，逐项完成权限、挂载边界与类型检查并哈希普通文件，同时按名称排序的深度优先顺序将节点摘要流式折叠为 v3 目标令牌。删除确认结果仅保留 REST 响应所需的根元数据和目标令牌，不保留完整目标树清单。最终值仍为 64 字符小写十六进制字符串，调用方必须将其视为不透明值。该变更不改变存储锁范围、目录扫描次数、文件哈希次数或 REST 请求与响应结构。`DELETE /api/v1/files/{path}` 必须同时提交模式、策略令牌和目标令牌。原子捕获前发现完整策略或目标变化时返回 `409 DELETE_POLICY_CHANGED` 或 `409 DELETE_TARGET_CHANGED`，不会提交业务状态；捕获开始后使用禁止覆盖的源端暂存、完整回收站副本清单和句柄锚定、权限不宽于 `0700` 的隔离目录完成删除。无法安全回滚的未提交结果返回 `500` 并保留恢复证据；已提交但物理清理未完成时返回成功状态与清理警告。MnemoNAS 存储锁会串行化平台内操作，但不覆盖同 UID 进程直接修改文件系统、特权进程并发挂载、进程崩溃或断电。后台保留任务会同时清理过期版本与到期回收站项目；回收站界面使用每个项目持久化的到期时间，并明确容量上限可能导致提前清理。
