...
- 文件页面会按当前策略明确区分“移入回收站”和“永久删除”，并在策略未知时停用删除而不影响浏览。文件列表为实际条目返回对象身份令牌；删除确认通过 `POST /api/v1/files-delete-intents` 提交所选条目的观察身份，并原子取得完整策略与目标树令牌。同一路径在点选后被新文件或目录替换时，即使类型、大小和修改时间相同，服务端也会拒绝旧身份。目标令牌现采用 v3 分层 SHA-256 Merkle 表示：最终令牌绑定规范化根路径与完整路径层级；每个节点摘要绑定文件或目录的域分隔标记、完整权限模式（`mode`）、大小、纳秒级修改时间、对象身份和快照中的内容哈希字段，普通文件的该字段承载实际内容摘要；目录节点按子项名称排序，并依次组合子项名称与摘要。删除确认先在短暂持有存储读锁时采样完整策略与变更纪元（`mutation epoch`），随后释放锁，在锁外对请求中的全部目标树执行严格遍历，逐项完成权限、挂载边界与类型检查并哈希普通文件，同时按名称排序的深度优先顺序将节点摘要流式折叠为 v3 目标令牌。扫描完成后会再次核对变更纪元；若其发生变化，则丢弃整批扫描结果并执行有限重试，必要时在存储读锁保护下完成保底扫描。删除确认结果仅保留 REST 响应所需的根元数据和目标令牌，不保留完整目标树清单。最终值仍为 64 字符小写十六进制字符串，调用方必须将其视为不透明值。仅就删除确认阶段而言，无竞争时每个目标的目录扫描次数和普通文件哈希次数不变；发生竞争时，被丢弃的尝试会增加相应次数。变更纪元不参与 v3 目标令牌计算，也不出现在 REST 请求或响应中；现有 REST 请求与响应结构保持不变。`DELETE /api/v1/files/{path}` 必须同时提交模式、策略令牌和目标令牌。原子捕获前发现完整策略或目标变化时返回 `409 DELETE_POLICY_CHANGED` 或 `409 DELETE_TARGET_CHANGED`，不会提交业务状态；捕获开始后使用禁止覆盖的源端暂存、完整回收站副本证明和句柄锚定、权限不宽于 `0700` 的隔离目录完成删除。每个普通文件副本发布后，复制流程会核对复制字节数，并通过仍打开的目标句柄完整哈希已发布内容，将结果与复制流摘要比较；回收站副本发布后的首轮源端与目标端完整内容哈希保持不变；随后执行不读取文件内容的完整源树和目标树元数据与身份复核，并在业务状态提交前最终复核两端挂载边界。删除确认、提交时的目标令牌重验、暂存捕获、首轮复制证明以及源暂存树移入隔离区前后的内容证明均保持不变。业务状态提交后、隔离源递归移除前，服务端还会再次完整哈希回收站副本；副本漂移会保留隔离源并返回已提交的清理警告。在无竞争且成功的单个普通文件删除中，删除确认与后续提交调用的回收站路径执行 9 次完整内容哈希加 1 次复制读取，其中一次哈希用于证明已发布目标与复制流摘要一致，最后一次哈希用于物理源清理前的副本复核；永久删除路径仍执行 5 次完整内容哈希。竞争、失败处理或回滚可能增加文件读取次数。服务端会在暂存前为尚无摘要的普通文件见证捕获内容哈希。改名后的暂存路径若被替换，`StagePath` 只记录与该摘要一致且完成文件及父目录同步、身份与内容复核的普通文件恢复副本；目录见证或无法确认的恢复保持空路径，`InspectionPaths` 列出需要人工检查的实际位置。此次调整不改变 REST 请求或响应、v3 目标令牌及外部状态码映射。无法安全回滚的未提交结果返回 `500`；服务端仅在证据身份可确认时提供恢复路径；已提交但物理清理未完成时返回成功状态与清理警告。MnemoNAS 存储锁与变更纪元只覆盖平台内操作。同 UID 进程直接修改文件系统或特权进程并发挂载不会推进该纪元，对象身份、挂载边界、暂存对象复核和恢复安全检查会拒绝每个已观察到的变化；外部写入者不受存储锁约束，最后一次复核与文件系统调用之间发生的变化仍超出应用级事务边界；进程崩溃或断电仍超出实时“移入回收站”、回收站恢复及其他仅限进程内机制的事务边界；已位于回收站的项目在永久删除时使用 `.deleting` 下的 `prepared`／`committed` 持久化决策日志。启动恢复在保留清理和网络监听前执行：仅有 `prepared` 时安全回滚，存在 `committed` 时只允许前滚；日志、内容或元数据无法互证时拒绝可写启动并保留残留。后台保留任务会同时清理过期版本与到期回收站项目；回收站界面使用每个项目持久化的到期时间，并明确容量上限可能导致提前清理。
