...
- 新增 `client/` Flutter 跨平台客户端工程，以 Android 作为首个可用平台目标。当前源码已接入服务器连接与校验、Bearer 会话、单次 refresh token 轮换、安全会话存储、首页概览、文件浏览和基本文件操作、流式上传下载、传输取消、Android Storage Access Framework 导出、账户管理及 Issue 反馈入口；客户端门禁和真实 `nasd` API 文件生命周期冒烟已通过。真实 Android 设备验收、正式发布签名、后台或可恢复传输，以及 Linux 和 Windows 原生构建验证仍未完成，当前没有可用客户端版本。
