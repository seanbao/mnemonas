...
- 新增 `client/` Flutter 跨平台客户端工程，以 Android 作为首个可用平台目标。当前源码已接入服务器连接与校验、Bearer 会话、单次 refresh token 轮换、安全会话存储、首页概览、文件浏览和基本文件操作、流式上传下载、Android Storage Access Framework 导入与导出、账户管理及 Issue 反馈入口；客户端门禁和真实 `nasd` API 文件生命周期冒烟已通过。真实 Android 设备验收、正式发布签名、后台传输、可恢复上传，以及 Linux 和 Windows 原生构建验证仍未完成，当前没有可用客户端版本。
- Flutter 客户端新增前台持久下载账本。任务按服务端与用户隔离，记录稳定局部文件、下载身份、可靠偏移、目标和阶段，但不记录认证令牌。客户端重启后先按局部文件实际长度对账：未完成任务恢复为暂停状态，已完整写入 Android 私有载荷的任务等待重新选择目标，可能已发布到桌面目标但无法确认的任务标记为“结果待确认”。恢复请求必须同时满足服务端对象身份、`206 Partial Content`、精确 `Content-Range` 和总大小，才会追加内容。Android 先完成应用私有载荷，再以临时写授权选择保存位置；网络失败不会提前创建空文档，原生截断复制成功后才标记完成，客户端重启后要求重新选择位置。Android 上传文件选择改用 URI-only `ACTION_OPEN_DOCUMENT`，以临时读取授权按文件流式复制到应用私有目录，并提供准备进度和取消操作，避免插件把完整多选内容载入 Java heap。现有上传接口仍是一次完整文件 `POST`；进入发送阶段后断线会保守显示“结果待确认”，不会自动重放。
