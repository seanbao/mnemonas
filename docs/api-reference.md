...
Web 客户端会先同步短期下载会话，再使用 `Range: bytes=0-0` 和 `X-Mnemonas-Download-Probe: json-error` 进行有界错误探测，最后交由浏览器原生导航流式下载。文件和 ZIP 内容不会整体读入 JavaScript Blob。认证 ZIP 的探测请求在收集、权限和快照校验成功后返回空正文，不生成完整归档。
...
当前文件下载使用同一已打开普通文件句柄的元数据，不为生成响应头预先读取完整内容。FIFO、Unix socket 和设备文件等非普通文件会被拒绝，不会在打开时阻塞请求处理。
