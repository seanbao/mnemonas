...
- HTTPS 请求只解析 `__Host-` 名称，HTTP 请求只解析无前缀名称，两种模式不会相互回退。同一请求中同名 Cookie 出现不同值时，服务端拒绝认证。在允许使用 download Cookie 的请求上，只要 access 与 download Cookie 同时存在且原始值不同，服务端会在验证任一候选前拒绝认证；Web 客户端必须先通过下载会话端点同步两者。
...
| `404` | 未找到 |
| `409` | 资源冲突或操作当前不可执行 |
| `410` | 资源不可用、已过期、已禁用或达到下载上限 |
| `412` | 下载恢复身份或其他请求前置条件不满足 |
| `413` | 文件过大 |
| `429` | 已限流 |
| `507` | 用户或目录逻辑配额不足，或主机写入容量不足 |
| `500` | 内部错误 |
| `503` | 服务依赖不可用 |
...
只有明确归类为持久化或清理警告的失败才会返回成功状态。持久化回收站转移发生硬失败时返回 `500 INTERNAL_ERROR`，并保留恢复证据。调用方应同时检查 HTTP `Warning` 响应头和 JSON body。
...
- `quota_bytes = 0` 表示无限制。
- 大于零时，服务端配额检查会应用于非管理员 Web/API 上传、复制、移动、回收站恢复。管理员将历史版本恢复至受限用户的 `home_dir` 时，同样检查目标用户配额。
...
- 主机文件系统无法提供写入事务所需的安全暂存空间时同样返回 `507 Insufficient Storage`，但错误码为 `INSUFFICIENT_STORAGE`，不包含逻辑配额详情，也不返回 `Retry-After`。该结果与用户或目录配额超限不同。
- Web/API 与 WebDAV 共用同一进程级配额预留协调器和写入提交门。上传、复制、移动、回收站恢复和版本恢复会在配额协调临界区内重新计算当前逻辑用量，并原子计入所有匹配用户配额、目录配额及尚未完成的预留。用量扫描与协作式 HTTP 写入提交不会并发执行；配额协调临界区本身不读取请求体，也不执行业务写入。
- 复制、移动、回收站恢复和版本恢复会在最终提交门内重新计算源、目标、当前用量和覆盖差量，并原子替换准入时的预留；刷新失败时不执行存储变更。WebDAV 覆盖 MOVE 会把提交期间仍位于逻辑目录内的目标备份计入所需空间，直到备份清理完成。PUT 只在请求体到达 EOF 后取得提交门，并持有到存储事务返回，因此网络读取不会长期占用该门。
- 已知长度上传按 `max(0, 新大小 - 被覆盖大小)` 预留。未知长度上传从被覆盖大小开始，以 64 MiB 为上限逐批增长预留，且文件总大小仍受 10 GiB 上限约束；成功、失败、条件冲突和请求取消都会释放预留。达到配额边界时会额外探测一个字节，因此刚好填满后结束的请求成功，实际超出时才返回 `507`。
- 配额规则与限制采用请求准入时的快照；运行时热更新应用于后续请求。未知长度上传的每次增量预留仍会重新计算当前逻辑用量和其他未完成预留。
- 启用提醒通道时，Web/API 上传、复制、移动、回收站恢复和版本恢复的配额拒绝也会发送 `quota_exceeded` warning 事件。
...
| `HEAD` | `/api/v1/download/{path}` | 获取当前文件或历史版本的下载响应头，不返回正文 |
...
策略、目标令牌或 WebDAV 条件通过后，服务端以禁止覆盖的原子重命名把当前对象捕获到源端同一父目录下的随机暂存路径，并从该路径继续处理。原逻辑路径在此期间出现的新对象不会被复制、覆盖或删除。`permanent` 模式的最终物理清理会把已验证暂存对象移入权限不宽于 `0700` 的随机隔离目录，并通过服务端持有的目录句柄逐项删除。`trash` 模式会在 `.mnemonas/trash/.transactions/` 下记录 `prepared`、`copying`、`ready`、`committed` 和 `completed` 检查点。新建容器使用权限为 `0600` 的规范化 owner marker 绑定 `prepared` 日志与持久身份；`copying` 在载荷复制前绑定已同步的操作专属容器，且仅在 marker 移除及目录同步完成后开始复制。`ready` 只在完整副本验证后发布。匹配的 `trash_operations` 发件箱行与回收站元数据及文件索引变更在同一事务内提交。分享与收藏参与者使用操作级回执和原删除操作所有权，使恢复重放保持幂等；删除完成后的显式分享或收藏变更会阻止旧恢复覆盖新意图。

逻辑提交前无法安全回滚时，REST 返回 `500 INTERNAL_ERROR`，不写入删除活动；内部暂存对象或配对证据可能为恢复处理而保留，具体主机路径不会写入 API 响应。`permanent` 模式在逻辑提交后未完成隔离区清理时，返回带 `warning=true` 的 `200 OK`、`199 MnemoNAS "delete cleanup incomplete"`，并在删除活动中记录 `cleanup_warning=true`。`trash` 模式只有明确归类为持久化不完整的失败才会返回成功，并附带 `199 MnemoNAS "workspace mutation persistence incomplete"` 和 `persistence_warning=true`。若可见变更完成后无法确认终态日志移除已持久化，服务端会保留或重建 `completed` 检查点并启用恢复门禁；当前响应仍使用持久化警告，但恢复完成前不会接受新的存储变更。容量清理在持久转移完成且终态日志已可靠清理后执行；失败时返回成功，并附带 `199 MnemoNAS "trash delete cleanup incomplete"` 和 `trash_cleanup_warning=true`。适用的持久化与容量警告可以同时返回。

`trash` 模式的日志、参与者、回执、发件箱、源端或目标端发生硬失败时返回 `500 INTERNAL_ERROR`。该失败可能发生在持久提交点之后；恢复证据会被保留，后续存储变更也会被阻止，直至恢复成功。持久提交点后的 `500` 不表示操作已经回滚：存在匹配 `trash_operations` 行时，转移只会前滚。调用方应在恢复完成后重新读取回收站和目标路径，再决定是否重试。WebDAV 对相应的纯警告结果返回 `204 No Content`，对转移硬失败返回 `500 Internal Server Error`。

服务端会在删除意图、删除前复核、跨根复制和受检清理期间重新读取主机挂载表。挂载表无法读取、包含非法路径，或目标跨越工作区根目录下的嵌套挂载边界时，流程停止。对象捕获前的冲突返回 `409 Conflict`。`permanent` 模式在捕获后无法安全回滚时形成恢复残留，逻辑提交后的同类问题形成清理警告。`trash` 模式在检查点发布后发生硬失败时会保留持久证据并要求恢复。仅在目标边界仍可验证时清理已复制副本，否则保留内部副本以避免跨越新挂载。工作区根目录自身可以是挂载点；该限制只适用于其下的嵌套挂载。

存储写锁只串行化通过 MnemoNAS 执行的操作。同 UID 进程直接修改文件系统和特权进程并发挂载仍不在该边界内。证据有效时，`.transactions` 检查点、`trash_operations` 发件箱、owner marker 和参与者回执会为实时回收站转移期间的进程崩溃与断电提供启动恢复。若 marker 在创建后、完整写入并同步前损坏，恢复无法证明容器所有权，会保留证据并阻止可写启动，直至人工核对。对象捕获和禁止覆盖回滚可能改变 ctime 或父目录时间戳。服务端不会移动或删除身份未知的替换物，也不会自动清理所有权无法确认的内部残留。
...
`GET /api/v1/download/{path}` 默认返回文件字节。`HEAD /api/v1/download/{path}` 对当前文件和历史版本执行相同的路径、权限与快照打开检查并返回对应响应头，但不返回正文；ZIP 归档不支持 `HEAD`。支持的查询参数：

- `download=true`：最多出现一次，用于强制附件文件名。
- `version=<hash>`：最多出现一次，用于下载历史版本。
- `archive=zip`：最多出现一次，用于将目标路径下载为 ZIP。

Web 客户端会先同步短期下载会话，再使用 `Range: bytes=0-0` 和 `X-Mnemonas-Download-Probe: json-error` 执行有界错误探测，最后交由浏览器原生导航流式下载。文件和 ZIP 内容不会整体读入 JavaScript Blob。认证 ZIP 的探测请求在收集、权限和快照校验成功后返回空正文，不生成完整归档。

Web UI 的一次批量下载最多提交 20 项，且最多包含一个文件夹；普通文件可与该文件夹一同提交。文件夹必须单独生成 ZIP，JavaScript 并发仅限制错误探测和浏览器提交阶段，不表示浏览器实际传输并发。浏览器提交使用随机命名、带 sandbox 且无 `src` 的同源隐藏导航上下文；实际 URL 只在设置 `referrerpolicy="no-referrer"` 的短期目标链接中存在，并在触发导航后立即清除。该链接不设置会改变命名目标解析的 `noopener` 或 `noreferrer`。成功提交的导航上下文在页面生命周期结束时清理，非下载错误响应加载完成后立即清理。单页最多保留 64 个活动或待提交上下文；达到上限后新提交会在网络预检或公开票据签发前被拒绝，并提示刷新页面后继续。

当前文件的 `GET` 和 `HEAD` 响应在平台能够生成对象身份时包含 `X-MnemoNAS-Download-Identity`。该值直接使用同一已打开普通文件句柄的 `deleteIdentityToken`，是不透明的对象与元数据身份，不是内容哈希。生成该响应头不会预先读取完整文件。FIFO、Unix socket 和设备文件等非普通文件会被拒绝，不会在打开时阻塞请求处理。

恢复当前文件下载时，客户端应保存初始响应中的下载身份，并在后续请求中恰好携带一次 `X-MnemoNAS-If-Download-Identity`。需要追加字节时，`Range` 只随 `GET` 发送；`HEAD` 会忽略 `Range` 并返回完整表示的响应头。服务端先打开本次请求使用的普通文件快照，再区分大小写地逐字节比较请求值与该快照的下载身份。匹配的 Range `GET` 继续使用现有 `http.ServeContent` 语义，例如返回 `206 Partial Content` 和 `Content-Range`；不匹配、请求头重复，或当前平台无法生成身份时返回 `412 Precondition Failed`、`Content-Length: 0` 且不发送正文。身份可用时，`412` 响应中的 `X-MnemoNAS-Download-Identity` 是本次已打开快照的当前值，客户端必须丢弃旧的局部文件或重新开始下载。

弱 ETag 仍用于缓存重验证；恢复下载的对象连续性由 `X-MnemoNAS-Download-Identity` 与 `X-MnemoNAS-If-Download-Identity` 单独约束。服务端在 GET 结束时重新检查已打开文件的身份，并在提交最后一段有界内容前完成确认。外部写入在流中改变同一文件时，响应会以短于声明 `Content-Length` 的形式终止，客户端不得把该响应提交为完整文件。
...
- 进程级非阻塞并发门限最多允许 4 个认证 ZIP 探测或完整写入同时执行。门限占满时会在访问文件系统前返回 `429 Too Many Requests`、`Retry-After: 1` 和 `ARCHIVE_DOWNLOAD_RATE_LIMITED`。
...
- 当前文件和历史版本的 `GET` 下载支持 Range 请求；`HEAD` 忽略 Range，ZIP 归档下载不保证支持 Range。
...
恢复前会确认版本元数据属于请求路径，并校验版本对象的 BLAKE3 摘要与 `{hash}` 一致。版本恢复复用与 API 上传和 WebDAV PUT 相同的暂存、已有目标原子交换、新目标禁止覆盖发布、索引提交和回滚事务，并检查所有匹配的目录配额；启用认证时，管理员执行恢复也会检查目标路径所属非管理员用户的 `home_dir` 配额。当前内容与目标版本不同时，当前文件会先保存为注释为 `before restore` 的安全版本；当前文件或版本对象超过 100 MiB 硬上限时返回 `413 Payload Too Large`，不会修改目标。

并发写入暂存槽占满时返回 `429 Too Many Requests`、`Retry-After: 1` 和 `WRITE_BUSY`；主机可用空间不足以安全暂存事务时返回 `507 Insufficient Storage` 和 `INSUFFICIENT_STORAGE`，且不返回 `Retry-After`。事务期间目标变化返回 `409 Conflict`；恢复门禁、嵌套挂载或不支持原子重命名的布局返回 `503 Service Unavailable`。在内容或索引提交失败且回滚成功时，接口返回失败状态，原文件和索引保持不变，也不记录成功恢复活动。发布后的运行时暂存项无法安全清理时会保留恢复证据、启用门禁并返回 `503 Service Unavailable`；只有不包含恢复门禁或其他硬状态的纯持久化 `warning` 才返回 `200 OK`、`Warning: 199 MnemoNAS "workspace mutation persistence incomplete"` 和 `version restored with persistence warning`。
...
永久删除回收站项目会在暂存和递归清理前检查回收站根目录下的嵌套挂载。含分享或收藏恢复载荷的项目还会在任何物理变更前，以只读方式验证规范载荷、参与者存储和恢复证据。`DELETE /api/v1/trash/{id}` 遇到嵌套挂载、无法验证挂载表或参与者预检失败时保留项目内容与元数据；挂载冲突返回 `409 Conflict`，参与者不可用按服务端错误处理。

永久删除的内容、SQLite 元数据和版本清理完成后，服务端按恢复载荷中的原删除操作 ID 精确清除分享与收藏所有权，再移除 purge 日志。参与者持久化首次出现警告时会重试一次。重试成功时项目保持已删除，响应带清理 warning；持续警告或硬失败同样保留已删除结果，但会保留 `committed` purge 日志并启用恢复门禁，后续存储变更在恢复完成前会被阻止。
...
如果回收站项目包含历史版本，且原路径已被当前文件占用，或其他回收站项目仍引用重叠的源/目标版本元数据路径，端点会在配额检查前返回 `409 Conflict`，且不发送配额提醒。目录恢复也会检查后代路径的重叠版本元数据。

恢复成功时返回 `200 OK`。可见恢复已经完成、仅参与者存储或终态日志清理的持久化状态无法确认时，响应包含 `data.warning=true`、消息 `file restored with persistence warning` 和 `199 MnemoNAS "workspace mutation persistence incomplete"`，恢复活动包含 `persistence_warning=true`。终态日志清理不确定时会保留或重建 `completed` 检查点，并阻止新的存储变更，直至恢复成功。分享与收藏路径依据操作快照、原删除操作所有权及操作级回执迁移；相同操作的重放不会覆盖其后发生的变更，删除完成后的显式分享更新、删除或收藏变更会阻止旧恢复复活对应状态。

日志、参与者、回执、发件箱、源端或目标端发生硬失败时返回 `500 INTERNAL_ERROR`。该响应可能发生在持久提交点之后，不表示回收站项目仍存在或目标路径仍不存在。存在匹配 `trash_operations` 行时，转移只会前滚。恢复 `completed` 操作时，流程会验证文件系统终态，并且只重试回执清理、发件箱确认和日志清理，不重放参与者 Apply。调用方应在恢复完成后重新读取回收站和目标路径，再决定是否重试。
...
搜索结果受配置的 `home_dir` 限制。本次搜索的根目录本身不作为结果返回。

查询参数：

- `q`：必填搜索词，必须是有效 UTF-8，去除首尾空白后最多包含 100 个 Unicode 码点，并且必须恰好出现一次。
- `limit`：最大结果数量。默认值为 50，最大值为 100，最多出现一次。

`count` 等于当前响应中的结果数量，不表示全部匹配项总数。接口当前不提供游标、总数或后续页；达到 `limit` 时，调用方应缩小搜索词，而不应把结果视为完整集合。
...
风险原因用于标识无密码、长期、宽范围文件夹、无限制、长期未下载或即将过期的链接。已启用分享在创建后 30 天从未成功签发下载票据会报告为 `unused_enabled`；已启用分享的最近逻辑下载超过 90 天会报告为 `stale_enabled`。
...
| `POST` | `/api/v1/public/shares/{share_id}/download-ticket` | 为一个文件或 ZIP 归档签发下载票据 |
| `GET` | `/api/v1/public/shares/{share_id}/download?ticket=...` | 使用票据下载分享文件或分享文件夹 ZIP |
| `GET` | `/api/v1/public/shares/{share_id}/download/{path}?ticket=...` | 使用票据下载分享目录中的条目或 ZIP |
...
`client_nonce` 必填，必须是 32 个随机字节的 43 字符规范 base64url 无填充编码。Web 客户端使用 Web Crypto 生成该值，并在同源 `localStorage` 的版本化键中复用。该值只是稳定客户端标识符，不是秘密或授权凭据，不应写入下载 URL 或日志。`path` 可省略，`archive` 可省略且非空时只接受 `zip`。

成功响应包含 `ticket` 和 RFC 3339 格式的 `expires_at`。`ticket` 是 URL-safe 的签名不透明值，长度不超过 256 个字符，并绑定分享、下载目标、归档模式和签发时的关键状态。服务端使用不同的 HMAC 域，根据分享 ID 与 `client_nonce` 分别派生 128 位 binder ID 和 256 位 binder 值；票据使用独立的随机 128 位 ID，只保存 binder ID 和 binder 值摘要，不保存 `client_nonce` 或 binder 值。同一分享与同一 `client_nonce` 的后续签发会刷新同一个 `Path=/`、`HttpOnly`、`SameSite=Strict` cookie；不同分享派生不同 cookie。cookie 名以 binder ID 的 32 字符小写十六进制形式结尾。HTTPS 使用带 `Secure` 的 `__Host-mnemonas_share_download_<binder-id>`，HTTP 使用 `mnemonas_share_download_<binder-id>`。cookie 与票据同时过期，最长有效 24 小时，且不会超过分享自身的过期时间。

签发端点对当前请求携带的结构合法 binder cookie 执行局部软限制：HTTP 只统计普通前缀，HTTPS 同时统计普通前缀和 `__Host-` 前缀。目标 binder cookie 必须恰好出现一次且值正确；重复或无效目标 cookie 会以 `429 Too Many Requests`、`Retry-After: 1` 和 `DOWNLOAD_TICKET_RATE_LIMITED` 拒绝。目标 binder 尚不存在且计数已达到 32 个时，也会在文件系统预检和逻辑下载计数保留前返回相同错误；正确的既有目标 binder 在达到或超过软限制后仍可刷新。因此，同一分享的正常重复下载不会持续增加 cookie 数量。所有票据签发还共用容量为 4 的进程级非阻塞并发门限，该门限覆盖 ZIP 预检；门限占满时使用相同的 `429` 响应。使用不同新 `client_nonce` 的并发请求可能基于同一旧 cookie 快照形成受该门限约束的短暂软上限超出。该限制不属于服务端持久会话计数；丢弃 binder cookie 的客户端无法再使用对应票据。
...
- `max_access` 是逻辑下载会话上限，`access_count` 是已成功签发的下载票据数。`max_access > 0` 且计数已达上限时，票据签发返回 `410 Gone` 和 `SHARE_ACCESS_LIMIT_REACHED`。
...
- 公开元数据、密码验证和文件夹列表不递增 `access_count`。下载票据完成权限、目标类型和 ZIP 预检后，以原子方式保留一次逻辑下载计数；同一票据的完整下载、Range 请求和断点续传不重复计数。
- `items?path=` 和 `download/{path}` 中的子路径相对于分享文件夹根目录。
  文件夹列表 `path` 查询参数最多出现一次。
  控制字符和独立的 `.` 或 `..` 路径段无效，合法名称中的连续点号（例如 `foo..txt`）仍有效。
  无效子路径不签发票据，也不递增 `access_count`。
- 文件夹列表响应中的 `path` 和 `items[].path` 为相对于分享文件夹根目录的规范路径，不以 `/` 开头；根文件夹响应使用空 `path`。响应只包含对分享所有者仍可见的直接子项。
- 下载 GET 请求必须恰好包含一个 `ticket` 查询参数，并携带签发时设置的 binder cookie。服务端会校验票据签名、binder、下载目标和分享关键状态。缺失或重复参数返回 `401 DOWNLOAD_TICKET_REQUIRED`；格式、签名、binder 或目标不匹配返回 `401 DOWNLOAD_TICKET_INVALID`；票据过期返回 `410 DOWNLOAD_TICKET_EXPIRED`；分享关键状态已变更返回 `401 DOWNLOAD_TICKET_STALE`。
...
- 在公开下载端点上设置 `archive=zip` 最多一次，可将分享文件夹根、子文件夹或文件下载为 ZIP。
  公开 ZIP 归档返回 `application/zip`，不保证支持 Range，会跳过对分享所有者不再可见的条目，并限制为最多 10000 个条目和 20 GiB 文件内容。归档只读取已验证的普通文件快照；管道、套接字和设备文件会被拒绝。
  ...
  公开 ZIP 的票据签发预检与实际 ZIP 流共用容量为 4 的进程级全局并发门限。门限占满时返回 `429 Too Many Requests`、`Retry-After: 1` 和 `DOWNLOAD_TICKET_RATE_LIMITED`。
...
- 密码失败限流按分享 ID 和客户端地址分桶。同一分桶最多执行一次并发 bcrypt 校验，进程内最多同时执行 8 次公开分享 bcrypt 校验。每个分享最多保留 128 个活跃客户端失败状态，进程内总计最多保留 4096 个；仍在失败窗口内、已锁定或正在校验的状态不会为新客户端腾出空间。局部或全局容量用尽、同一分桶已有校验或 bcrypt 门限占满时，新请求返回 `429`，且不会清除既有失败计数。超过 72 字节的密码在进入限流状态和 bcrypt 前返回 `400 PASSWORD_TOO_LONG`。默认忽略转发头；仅当 `server.trusted_proxy_hops > 0` 且直连来源为 loopback 或属于 `server.trusted_proxy_cidrs` 时，才使用转发头。
- 兼容路径 `GET /s/{share_id}` 和 `POST /s/{share_id}` 提供相同公开 JSON 行为，适合脚本或非 SPA 直接调用。
- 兼容路径 `GET /s/{share_id}/items`、`POST /s/{share_id}/download-ticket`、`GET /s/{share_id}/download` 和 `GET /s/{share_id}/download/{path}` 提供相同文件夹列表、票据和下载行为，适合脚本或非 SPA 直接调用。
...
- 添加收藏时，目标路径必须在提交时存在且允许当前用户执行精确读取；目标不存在时返回 `404 Not Found` 和 `FILE_NOT_FOUND`。路径检查与收藏持久化在同一存储变更租约内完成，以避免与并发删除产生失效收藏。
...
- 创建、删除和启用状态更新会写入 `share` 或 `unshare` 活动。`details` 包含分享类型、权限、密码要求、过期时间和下载上限等复核元数据；启用状态更新还包含 `enabled` 和 `previous_enabled`。这些详情不包含分享密码、公开 URL 或分享 ID。
...
目录配额和访问规则更新会热应用到 Web/API 和 WebDAV 运行时。已完成准入的在途写入继续使用其配额限制、配额规则和访问规则快照；Web/API 上传的路径访问判定以准入检查为准。WebDAV PUT 还会在提交前重新认证用户，并根据该请求准入时的访问规则快照复核当前身份、角色和主目录对应的写权限，同时复核当前 DAV 锁。WebDAV handler 重建会立即发布新代，后续请求直接使用更新后的配置；在途请求和路径变更回调继续引用旧代，引用归零后旧代才异步关闭。前缀、认证、访问规则、配额或只读设置更新后，未过期 DAV 锁仍以相同 token、存储路径、深度和到期时间生效。
...
- “用户管理 > 目录与访问”页的目录配额行式输入会用双引号包裹包含空格或双引号的路径；路径内双引号写作 `\"`，例如 `"/Family Photos" 500 GB`。
- 目录权限和分享路径策略使用结构化路径输入框，路径含空格或双引号时直接填写路径文本，不需要手动加引号。

Web 设置页会基于当前未保存草稿生成分享策略覆盖摘要。摘要展示默认过期时间、默认下载上限、路径策略数量、要求密码的路径数量、限制创建/维护者范围的路径数量、宽松默认值或路径策略的注意项，以及根路径规则、未继承上级限制的最具体路径规则、子路径放宽上级有效期/下载次数/允许创建者范围的规则和等价重复规则等整理建议。
...
`POST /api/v1/settings/access-reviews` 接受“用户管理 > 目录与访问”页生成的目录权限矩阵或未保存规则预览摘要。服务端使用当前认证账户作为 `reviewer`，设置 `reviewed_at`，并持久化最多最近 100 条记录。相同复核人、路径、标题和预览标记的记录会由新记录替换旧记录。
...
- `share_default_policy` 检查新建分享的默认过期时间和默认下载次数。
  启用分享时，没有默认过期时间、值长于 `720h` 或默认下载次数不限制为 `warning`，负数默认值为 `block`。
...
- HTTP 条件头遵循实体标签比较规则：`If-Match` 使用强比较，弱实体标签不能满足该条件；`If-None-Match` 使用弱比较。两者支持单独的 `*` 和严格的非空逗号分隔实体标签列表。条件 `PUT` 在读取请求体前求值条件，并把当时的目标删除身份绑定到最终写入；流式读取期间目标再次变化时，带任一写入条件头的请求返回 `412 Precondition Failed`，无条件请求返回 `409 Conflict`，较新的对象保持不变。条件 `DELETE` 会在同一存储写锁内完成目标树写权限复核后，再读取所需目标属性并求值条件；权限复核失败时不计算目标内容哈希，也不删除目标。
- WebDAV `DELETE` 遵循当前删除策略。`permanent` 模式在逻辑提交后未完成隔离区清理时，返回带 `delete cleanup incomplete` 的 `204 No Content`。`trash` 模式发生参与者或回执持久化不完整时，返回带 `workspace mutation persistence incomplete` 的 `204 No Content`；转移完成后的容量清理失败使用 `trash delete cleanup incomplete`。持久转移发生硬失败时返回 `500 Internal Server Error`，保留恢复证据，并阻止后续存储变更，直至恢复成功。
