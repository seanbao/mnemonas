...
用户响应包含 `id`、`username`、`email`、`role`、`groups`、`disabled`、`home_dir`、`created_at`、`updated_at`、可选 `last_login_at`、`quota_bytes` 和 `used_bytes`。列表响应还会返回 `quota_history_available` 和 `quota_history`；服务端会保留最近 64 条配额聚合变化快照，历史不可写时该标记为 `false`，用户列表仍会返回。
