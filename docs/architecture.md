...
- Android 上传选择使用 Storage Access Framework 的 `ACTION_OPEN_DOCUMENT`。原生层只向 Dart 返回 `content` URI、显示名称、MIME 类型和可选大小，不返回完整文件字节，也不持久占用读取授权。上传前，原生层以固定大小缓冲区把单个来源流式复制到应用私有目录并执行 `fsync`，同时报告进度并接受取消请求，避免大文件或多选文件同时占用 Java heap。桌面端继续使用普通文件路径。
- 当前上传接口仍以完整文件流作为一次 `POST` 请求，不支持分块恢复或提交状态查询。上传进入发送阶段后发生连接中断时，客户端会保守地将任务标记为“结果待确认”，不会自动重放可能已经提交的上传。
...
该架构说明只描述当前源码边界。持久下载账本目前由前台 Dart 协调器驱动，Android 原生后台执行器、可恢复上传会话和跨进程任务租约尚未完成。SAF 文档提供方可能在原生复制失败后保留已创建的空文件或局部文件，客户端不会自动删除无法确认所有权的外部文档。桌面目标发布也尚未具备跨进程禁止覆盖原语、持久发布日志和目录同步。Flutter 客户端仍处于开发阶段，尚未发布任何可用版本；Android 真实设备验收、升级验证和独立发布签名尚未完成，Linux 与 Windows 也尚未形成经过验证的可发布构建。
