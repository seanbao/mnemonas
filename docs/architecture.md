...
- 应用进入 `hidden`、`paused` 或 `detached` 生命周期状态时，控制器会同步认领传输中心内仍在运行的任务，再取消对应网络请求或 Android 文档复制。持久任务会保留应用私有载荷和断点进度，并进入暂停或等待重新选择保存位置状态。临时文件预览、历史版本预览或下载会取消，下载局部文件随后清理；独立的 SAF 上传准备弹层也会取消原生导入并清理局部文件。回到前台只解除本轮生命周期防抖，不会自动继续任务。
- 版本历史入口面向具备规范化路径、具体读取权限和版本化标记的普通文件，不要求目录条目预先包含内容哈希。版本 API 返回并建立该路径的当前 64 字符小写 BLAKE3 内容身份；目录条目已携带身份时，两者还必须一致。响应会严格校验路径、连续版本号、哈希、大小、时间和说明文本。当前版本卡只展示身份与元数据，预览和下载操作只提供给历史版本；当前文件操作仍由文件页承担。历史下载使用认证流式请求，并要求响应的 strong ETag 精确等于所选版本哈希且长度一致；Bearer token 不进入外部打开或保存 URL。管理员恢复在提交前重新读取父目录和版本历史，核对初次版本响应建立的当前内容身份及所选历史版本。恢复 `POST` 不自动重放；连接中断、超时、不可解析响应或 `5xx` 会标记为“结果待确认”。服务端未提供当前身份 CAS 参数，因此提交前核对与写请求之间仍存在 TOCTOU；服务端恢复时会把当时的当前内容保存为安全版本。

该架构说明只描述当前源码边界。持久上传与下载账本目前都由前台 Dart 协调器驱动；Android 原生后台执行器、通知操作和跨进程任务租约尚未完成。后台执行的目标边界是 Android 14 及以上使用用户发起的数据传输任务（UIDT），Android 7 至 13 使用 WorkManager 长任务回退，并由 Application 级单一 FlutterEngine 执行 Dart 协调逻辑；接入该边界前，必须先把任务账本改为支持 revision/CAS、持久任务指令和 fencing token 的事务存储。SAF 文档提供方可能在原生复制失败后保留已创建的空文件或局部文件，客户端不会自动删除无法确认所有权的外部文档。桌面目标发布也尚未具备跨进程禁止覆盖原语、持久发布日志和目录同步。Flutter 客户端仍处于开发阶段，尚未发布任何可用版本。Release 构建已具备失败关闭签名校验，Android 16 真机也已覆盖基本安装、系统返回、进程重启会话恢复和 Debug 同签名升级；正式密钥托管、受控 Release 候选包、完整 Android 设备/故障矩阵，以及 Linux 与 Windows 可发布构建仍未完成。
