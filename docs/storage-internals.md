...
MnemoNAS v2 采用 **原生文件 + 版本历史** 混合架构：用户文件直接存储为原生文件（保持可读性和兼容性），历史版本使用 CAS（内容寻址存储）保存。元数据通过 SQLite 管理，确保高效查询和事务一致性。
...
| 特性 | MnemoNAS v2 | 传统 NAS (TrueNAS/Synology) | 纯 CAS 方案 |
|------|-------------|---------------------------|------------|
| **当前文件存储** | 原生文件 | 原生文件 | CAS 对象 |
| **版本存储** | CAS 对象 | 快照 | CAS 对象 |
| **文件可读性** | ✅ 拔盘即用 | ✅ 拔盘即用 | ❌ 需要软件 |
...
MnemoNAS v2 相比 v1 的主要变化：

| 方面 | v1 (CAS-only) | v2 (混合架构) |
|------|--------------|---------------|
| 当前文件 | CAS 存储 | 原生文件 |
| 拔盘可读 | ❌ 需要软件 | ✅ 直接可用 |
| 元数据 | JSON 文件 | SQLite |
