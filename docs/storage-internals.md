...
回收站副本成功发布后，服务端先对源暂存树和目标副本各执行一次完整内容哈希，并建立包含路径、类型、大小、权限、对象身份和摘要的完整复制证明。首轮内容证明完成后，服务端紧接着执行不读取文件内容的完整源树与目标树遍历，复核条目集合、路径映射、类型、大小、权限、修改时间与对象身份，随后最终复核两端挂载边界，才允许提交业务状态。删除确认、提交时的目标令牌重验、暂存捕获、该首轮源端与目标端复制证明，以及源暂存树移入隔离区前后的内容证明均保持不变。最终物理删除会把已验证源暂存对象移入同一父目录下权限不宽于 `0700` 的随机隔离目录，并通过服务端持有的目录句柄逐项核对后移除。硬链接组中的第一个条目使用完整身份令牌核对，后续条目同时要求与清单中的同一 inode 匹配，以容纳本次删除前序链接引起的 ctime 变化。未知条目、同名替换、身份变化或新挂载都会停止递归删除并保留隔离内容。

在无竞争且成功的单个普通文件删除中，删除确认与后续提交调用的回收站路径从 9 次完整内容哈希加 1 次复制读取降为 7 次完整内容哈希加 1 次复制读取；永久删除路径仍执行 5 次完整内容哈希。竞争、失败处理或回滚可能增加文件读取次数。此次调整不改变 REST 请求或响应、v3 目标令牌、错误映射和残留处理语义。
