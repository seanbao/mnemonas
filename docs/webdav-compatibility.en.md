...
Authentication: `auth_type = "users"` accepts MnemoNAS user credentials, maps regular users' mount root to their `home_dir`, applies matching directory access rules for shared paths, makes guest accounts read-only, and enforces user quotas for PUT/COPY. `auth_type = "basic"` remains the global service-credential compatibility mode.
