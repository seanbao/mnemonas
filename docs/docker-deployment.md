...
脚本不会输出密码。既有部署会在对应管理员成功修改或重置密码后删除该文件。全新部署缺少该文件时，应通过容器日志排查。
...
默认会调用 Docker 检查 `ghcr.io/seanbao/mnemonas:1.2.3` 是否存在，并会重试短暂不可见的镜像 manifest；需要调整时，可设置 `MNEMONAS_RELEASE_IMAGE_CHECK_RETRIES` 和 `MNEMONAS_RELEASE_IMAGE_CHECK_SLEEP_SECONDS`。
...
需要保留临时下载目录用于排查失败时，可省略 `--artifact-dir` 并传入 `--keep-artifacts`；脚本会在成功或失败退出时输出保留目录。
