...
镜像内置的 `/app/mnemonas-healthcheck` 默认请求 `http://127.0.0.1:8080/health`。
如通过 `MNEMONAS_HEALTHCHECK_URL` 覆盖目标，取值必须是绝对 `http` 或 `https` URL，且不能包含空白字符、控制字符、嵌入凭据或 fragment。
校验失败时，healthcheck 诊断只输出脱敏后的 URL 形状；嵌入凭据、query 和 fragment 不会写入容器日志。
...
该目标构建 `mnemonas:latest`，然后通过 `scripts/docker-smoke.sh` 仅在 `127.0.0.1` 发布临时端口，检查 `/health` 和前端根页面。通过命令参数或 `MNEMONAS_DOCKER_SMOKE_IMAGE` 覆盖烟测镜像时，镜像引用不能为空、不能以 `-` 开头，且不能包含空白或控制字符。通过 `MNEMONAS_DOCKER_SMOKE_HOST` 覆盖发布主机时，只接受四段数字形式的 `127.0.0.0/8` loopback IPv4 地址。HTTP 探测默认使用 `CURL_CONNECT_TIMEOUT=3` 和 `CURL_MAX_TIME=10`，可在慢速本地环境中按需增大。
