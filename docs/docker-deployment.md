...
建议优先使用明确版本标签。`latest` 只适合临时评估或明确接受自动升级风险的环境。Docker 预检会对未设置明确标签、未使用 digest 或使用 `latest` 的发布镜像给出提示，以便升级和回退时能定位到确定镜像。
...
release 镜像升级前应记录当前 `.env` 中的 `MNEMONAS_IMAGE` 标签，并完成一次备份。若升级后容器无法启动、核心流程异常或健康检查失败，可把 `.env` 中的 `MNEMONAS_IMAGE` 改回上一版本标签后回退：
...
回退只切换容器镜像，继续使用同一个宿主机数据目录和容器内 `/data/config.toml`。若新版本已执行不可逆数据迁移，应先按对应 release note 或备份恢复流程处理，不应直接用旧镜像读取迁移后的数据。
