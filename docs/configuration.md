...
> [!WARNING]
> MnemoNAS 尚未发布可用版本。配置格式仍可能在开发阶段调整，不提供已发布版本兼容性保证。
...
| `read_timeout` | duration | `"30s"` | 普通请求的总读取超时；浏览器/API 文件上传与 WebDAV PUT 将其用作每次读取请求体前刷新的空闲读取截止期 |
| `write_timeout` | duration | `"60s"` | 普通响应的总写入超时；浏览器/API 下载、文件上传响应与 WebDAV 响应将其用作每次写入前刷新的空闲写入截止期 |
...
浏览器/API 文件上传与 WebDAV PUT 会在每次读取请求体前，把连接读取截止期推进 `server.read_timeout`。只要请求体持续传入数据，上传就不会仅因总读取时长超过该值而中断；单次读取停滞超过该值时仍会超时。浏览器/API 文件上传仍受 API 默认 30 分钟请求总时限约束，其他请求体继续使用固定的总读取超时。

浏览器/API 下载、文件上传响应与 WebDAV 响应进入流式空闲写入模式时，会先清除连接或 HTTP/2 流继承的固定写入截止期，再在每次响应头、数据块或刷新操作前，把写入截止期推进 `server.write_timeout`。因此，首个响应较晚或持续产生数据的大文件和 ZIP 下载不会仅因总时长超过该值而被截断；单次写入停滞超过该值时仍会超时。其他 API 响应继续使用固定的总写入超时。反向代理还必须配置与预期传输时长和空闲边界一致的超时。

收到 `SIGINT` 或 `SIGTERM` 后，服务端会先停止存储告警监控并停止接收新连接，再最多等待 30 秒让在途 HTTP 请求完成。主进程只会在 `Shutdown` 完成后关闭 API、WebDAV 运行态锁和文件系统资源。排空超时或失败时，服务端会显式强制关闭剩余连接、记录错误，然后再释放这些主资源。
...
- `root/files` 与 `root/.mnemonas/write-staging` 必须属于同一个原子重命名域。启动时会在两个目录之间执行双向禁止覆盖重命名探针和双向原子交换探针；任一探针失败或平台缺少安全交换原语时，服务以存储布局配置错误拒绝启动。该检查不以 `st_dev` 相等作为充分条件，因此把任一目录单独绑定挂载到同一底层文件系统仍可能被拒绝。每次流式写入还会在读取请求体前和发布前复核挂载表，拒绝位于 `root/files` 嵌套挂载点内的目标，并返回 `503 Service Unavailable`。
...
- “用户管理 > 目录与访问”页会在保存前按已保存配额和当前草稿显示新增、修改和删除的目录配额摘要。行式输入中，包含空格或双引号的路径使用双引号包裹，路径内双引号写作 `\"`，例如 `"/Family Photos" 500 GB`。
...
- “用户管理 > 目录与访问”页会在保存前按已保存规则和当前草稿显示新增、修改和删除的目录权限规则摘要，并基于当前草稿展示规则数量、可读/可写主体、写权限路径和根路径或宽角色授权等覆盖关注项。用户矩阵和未保存规则预览可复制权限复核记录，并保留后端持久化近期复核历史；服务端历史不可用时回退当前浏览器记录。该页使用结构化规则编辑器，路径字段直接填写 MnemoNAS 逻辑路径，包含空格或双引号时不需要手动加引号。目录策略保存仅提交 `storage.directory_quotas` 和 `storage.directory_access_rules`，不会覆盖设置页维护的其他配置。
...
| `max_versioned_size` | int64 | `104857600` | 最大自动版本化文件大小（字节）；有效范围为 1 至 `104857600`，单文件覆盖不能绕过该硬上限 |
...
- 通过设置 API 更新 `webdav` 配置后，新 handler 会立即发布；后续请求使用新前缀、读写模式和认证配置，在途请求继续使用准入时的配置并在完成后释放旧 handler
...
当前开发构建不会启动 SMB/Samba 监听器。该配置段只保留给后续 SMB 网关侧车使用，启用后 `nasd --check-config` 会输出预览警告，健康页和诊断导出也会显示 SMB 运行态不可用。开发验证中的局域网挂载应继续使用 WebDAV。
