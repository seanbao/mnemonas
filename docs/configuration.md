...
Web 分享创建弹窗会在提交前展示策略来源、密码要求、有效期和访问次数的实际复核摘要，并标出路径策略上限收紧的项目。Web 设置页会在保存前展示分享功能、基础 URL、默认有效期、默认访问次数和路径策略相对已保存配置的变更摘要，并在覆盖摘要中列出根路径规则、未继承上级限制的最具体路径规则和等价重复规则等整理建议。分享列表会汇总需复核、无密码、覆盖较大、即将到期和长期未访问链接，提供对应筛选、当前范围复核记录写入、按分享类型筛选的复核历史跳转与需处理分享的停用入口。
