...
- 当 `[alerts] enabled = true` 且配置了 Webhook、Telegram、企业微信或 SMTP 邮件时，磁盘缺失、SMART 失败、温度过高、序列号不匹配或 SMART 不可用会发送 `disk_health` 事件。事件详情只包含聚合计数，不包含设备名、完整设备路径、序列号或 warning 文本。
