...
**自动生成行为：**
`username` 为空且 `auth_type = "basic"` 时，运行态使用默认值 `admin`。当 `auth_type = "basic"` 且 `password` 为空时，首次启动会自动生成 16 字符可读密码，至少包含小写字母、大写字母和数字，字符集会排除易混淆字符，并写入 `<storage.root>/secrets.json`。启动日志只提示凭据位置，不输出明文 WebDAV 密码。
