...
- 设置和管理入口按使用目标组织；默认页先显示状态与主要任务，低频网络参数使用渐进披露。CDC、数据面连接和其他部署参数保留在配置与诊断入口，不进入消费级 Web 表单。
- 页面按任务归属承载运行时配置：通知与磁盘健康设置位于设备状态页，周期校验计划位于备份与维护页，收藏开关位于收藏页。
- 能独立更新的运行时设置应使用领域级保存。通知、磁盘健康、周期校验计划和收藏开关分别只提交 `alerts`、`disk_health`、`maintenance.scrub` 和 `favorites`，不应覆盖其他设置快照。
