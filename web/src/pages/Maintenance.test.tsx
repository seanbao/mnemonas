  ...
  describe('backup jobs', () => {
    ...
    it('previews and runs a batch restore from the backup card', async () => {
      ...
      await waitFor(() => {
        ...
        expect(dispositions.getByText('冲突处置记录')).toBeTruthy()
        expect(dispositions.getByText('批量恢复只写入各自目标目录，不覆盖当前 storage.root；失败项和未通过校验项不能进入切换。')).toBeTruthy()
        expect(dispositions.getByText('可进入切换复核：只读校验通过，未发现覆盖当前 storage.root 的冲突。')).toBeTruthy()
      })
      ...
      expect(report).toContain('处置建议：可进入切换复核：只读校验通过，未发现覆盖当前 storage.root 的冲突。')
    ...
    it('localizes partial batch restore failure summaries', async () => {
      ...
      await waitFor(() => {
        ...
        const dispositions = within(screen.getByLabelText('批量恢复冲突处置记录'))
        expect(dispositions.getByText('可进入切换复核：只读校验通过，未发现覆盖当前 storage.root 的冲突。')).toBeTruthy()
        expect(dispositions.getByText('目标冲突未写入：保留现有目标目录，改为新的独立目录后重新生成批量预览。')).toBeTruthy()
      })
