...
- folder creation, upload, download, open, rename, move, copy, and deletion confirmation based on the server deletion policy; directory responses must return a canonical path matching the request, and system back on the Files page cancels selection before navigating to parent directories;
...
- separate Android identities for Release, Debug, and Profile; release APK/AAB generation requires signing configuration and keystore material outside the source checkout, rejects injected signing overrides, invalid private-key aliases, invalid certificates, and mismatched SHA-256 fingerprints, and CI exercises APK/AAB signing and bypass regressions with a temporary test key.
...
- An Android 16 physical device covers installation, sign-in, system-back hierarchy, session recovery after process restart, and a same-signature Debug update. Production-key custody, a controlled Release candidate, API 24/33/34 coverage, and the complete device/failure matrix remain incomplete.
